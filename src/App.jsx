...
if (rk.toLowerCase().trim().replace(/[%*\s]/g, “”) === k.replace(/[%*\s]/g, “”)) {
return row[rk];
}
}
}
...
<style>{`* { box-sizing: border-box; } input::placeholder { color: ${T.textFaint}; } input:focus { outline: none; border-color: ${T.gold} !important; } button { font-family: ${FONT_BODY}; cursor: pointer; } button:disabled { cursor: not-allowed; opacity: 0.5; } @keyframes spin { to { transform: rotate(360deg); } } @keyframes fadeIn { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } } .spin { animation: spin 1s linear infinite; } .card-enter { animation: fadeIn 0.25s ease-out; }`}</style>
...
const { holding, deltaShares } = item;
...
<>
<style>{FONT_IMPORT}</style>
<style>{`* { box-sizing: border-box; } input::placeholder { color: ${T.textFaint}; } input:focus { outline: none; border-color: ${T.gold} !important; } @keyframes spin { to { transform: rotate(360deg); } } .spin { animation: spin 1s linear infinite; }`}</style>
<div
style={{
minHeight: “100vh”,
background: T.bg,
