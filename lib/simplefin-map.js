...
function isTradeDescription(upper) {
  if (!TRADE_RX.test(upper)) return false;
  ...
  if (FEE_CHARGED_RX.test(upper)) return false;
  if (FOREIGN_TAX_RX.test(upper)) return false;
  if (DIVIDEND_RX.test(upper) && !REINVEST_RX.test(upper)) return false;
  if (INTEREST_RX.test(upper)) return false;
  if (REDEMPTION_RX.test(upper)) return false;
  return true;
}
