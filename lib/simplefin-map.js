...
// synthetic-id prefixes `sfbond-buy:` and `sfstock-delta:` are ALSO always
// re-derivable every sync (their whole `mapSimplefinPayload` logic reruns
// unconditionally from the current snapshot/qty state, independent of the
// 44-day transactions window `seenTradeSourceIds` is scoped to), so they are
// dropped unconditionally too, regardless of whether they show up in
// `seenTradeSourceIds`.
//
...
// `{ transactions, unmapped, consumedTickers, bondTradeLinks }` -- the same
// "never invent, surface what can't be resolved" shape as the rest of this
// file. `consumedTickers` MUST be forwarded to stockPositionDeltas (see the
// suppression note there). `bondTradeLinks` (sep/2026) carries bond/CD BUY
...
function reconcileTrades(account, netQtyByTicker = {}, liveTransactions = null) {
  ...
  return { transactions, unmapped, consumedTickers, bondTradeLinks };
}
...
export function mapSimplefinPayload(
  ...
  for (const account of accounts) {
    ...
    // Trade reconciliation runs BEFORE bond buy synthesis (sep/2026 --
    // previously the order was reversed): it produces the exact trades (real
    // date + real total), reports which tickers it consumed (so the
    // approximate delta candidate for those same tickers is suppressed
    // instead of being staged as a duplicate -- see the note in
    // stockPositionDeltas, the server's dupKey cannot catch it, the dates
    // differ), AND now also `bondTradeLinks` -- bond/CD trade rows with no
