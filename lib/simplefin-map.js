...
// case): a SimpleFin trade row for ticker T, side S, dated D is skipped
// ENTIRELY (no candidate, no unmapped row, no share of any delta) when
// `liveTransactions` already has a row of the SAME ticker T AND THE SAME
// SIDE S dated on or after D whose simplefinId is null/empty (a CSV/manual
// row) or starts with `sfstock-delta:` (a legacy holdings-diff estimate
// approved before this feature existed). The side match is mandatory: a
// sell proves nothing about whether an earlier buy of the same ticker was
// ever recorded (and vice versa) -- without it, an unrelated manual sell
// dated after an unrecorded real buy would silently swallow that buy with
// no transaction and no unmapped row, which is worse than the bug this rule
// fixes. Concretely: the user approved a VT buy back when this
...
function reconcileTrades(account, netQtyByTicker = {}, liveTransactions = null) {
  ...
  // row for ticker T, SIDE S, dated D is considered already covered by the
  // user's own records when `liveTransactions` has a row of the SAME ticker
  // AND THE SAME SIDE dated on or after D whose simplefinId is empty
  // (CSV/manual) or a legacy `sfstock-delta:` holdings-diff estimate
  // approved before this feature existed. Side match is mandatory (bugfix,
  // auditor-caught before this ever shipped): an unrelated manual SELL of a
  // ticker must never cover an unrecorded real BUY of the same ticker (or
  // vice versa) just because it happened later -- a sell proves nothing
  // about whether an earlier buy was ever recorded, it only reduces the
  // position. Without the side check, a live "sell VOO 08-01" (no
  // simplefinId) would have silently swallowed a real, never-recorded
  // "buy VOO 07-01" SimpleFin row with no transaction and no unmapped entry
  // -- exactly the kind of silent data loss this file exists to avoid. A
  // live row carrying a REAL SimpleFin transaction id never matches here --
  // that case is handled exclusively by the id-based `approvedSimplefinIds`
  // check above, matched exactly, not by date.
  const isAlreadyCoveredByLiveRecord = (ticker, side, date) => {
  ...
  for (const tx of txs) {
    ...
    // exact ticker+side.
    if (isAlreadyCoveredByLiveRecord(row.ticker, row.side, row.date)) continue;
