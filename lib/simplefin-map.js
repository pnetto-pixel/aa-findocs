...
// candidate instead of accumulating duplicates.
function computeBalanceCandidates(account) {
  ...
  for (const h of holdings) {
    ...
    // The CASH synthetic holding also has symbol === "" — must be excluded
    // here or the cash balance would double-count into Bank Bonds.
...
export function mapSimplefinPayload(payload) {
  ...
  for (const account of accounts) {
    ...
    balanceCandidates.push(...computeBalanceCandidates(account));
