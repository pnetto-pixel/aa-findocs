...
export function computeNetQty(transactions) {
  const net = {};
...
function stockPositionDeltas(account, netQtyByTicker = {}) {
  const asOf = unixToDateOnly(account?.["balance-date"]) || new Date().toISOString().slice(0, 10);
  const accountId = account?.id ?? null;
  const accountName = account?.name ?? null;
  const transactions = [];
...
function mapOneTransaction(tx, account, knownBondsByDescKey = new Map(), bondBindings = {}) {
  ...
  if (INTEREST_RX.test(upper)) {
    ...
    if (matches.length === 1) {
      ...
      // Two sources, in order: the metadata on the user's own buy
      // transactions (automatic, needs coupon+maturity+issuer saved), then
      // the persisted bind the user confirmed by picking a CUSIP in the
      // Fidelity Income dropdown. The second exists because the first can
      // never learn a bond whose buy has no coupon/maturity metadata — see
      // the function header.
      const bound = bondBindings && bondBindings[meta.descKey];
      const resolvedCusip =
        knownBondsByDescKey.get(meta.descKey) ||
...
export function mapSimplefinPayload(
  payload,
  { knownBondsByDescKey, netQtyByTicker, bondBindings } = {}
) {
  ...
  for (const account of accounts) {
    ...
    if (netQtyByTicker && typeof netQtyByTicker === "object") {
      const deltas = stockPositionDeltas(account, netQtyByTicker);
