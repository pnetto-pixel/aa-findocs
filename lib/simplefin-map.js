...
function semanticTradeKey(tx) {
  ...
  return ticker && (side === "buy" || side === "sell") && /^\d{4}-\d{2}-\d{2}$/.test(date)
    ? `${ticker}|${side}|${date}`
    : null;
}
...
// by ticker+side+date+economic total, choosing the globally closest cent
// difference first so repeated same-day trades pair deterministically.
  ...
  pending.forEach((candidate, candidateIndex) => {
    ...
    live.forEach((existing, liveIndex) => {
      ...
      for (const a of candidateTotals) for (const b of liveTotals) {
        ...
        if (difference <= 1) edges.push({ difference, candidateIndex, liveIndex });
      }
    });
  });
  edges.sort((a, b) => a.difference - b.difference || a.candidateIndex - b.candidateIndex || a.liveIndex - b.liveIndex);
