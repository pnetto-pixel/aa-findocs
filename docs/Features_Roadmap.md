...
- **Bug fix — Performance não reage a mudanças em Transactions** (jun/2026 — PR #53): Cache Redis de `perf-history` usava key fixa por usuário, ignorando mudanças nas transactions. Fix: cache key agora inclui FNV-1a hash das transactions elegíveis (`id|date|side|ticker|qty|price`). Qualquer adição, remoção ou edição gera hash diferente → cache miss → recalcula automaticamente. Cache bumped v11→v12.
- **Bug fix — positionRows qty errado em ticker com sells** (jun/2026 — PR #53): `positionRows` não ordenava transactions por data. Sells importadas via Fidelity (ordem decrescente) chegavam antes dos buys no array → qty ficava negativo → reset a 0 → sell ignorada. Fix: `[...transactions].sort()` por data no início do useMemo, igual ao que `computePerformance` já fazia no servidor.
- **UI — remover data redundante do card header** (jun/2026 — PR #53): "as of [date]" no cabeçalho do card "Portfolio Performance & Net Worth" duplicava a data já exibida no topo do app. Removido.
- **Bug fix — "Bank Bonds" ausente na mensagem de erro de classe elegível** (jun/2026 — PR #53): Mensagem "No transactions in eligible asset classes" em Performance.jsx não listava Bank Bonds. Corrigido.
