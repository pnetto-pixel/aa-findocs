...
**(4) Holdings mostra current value, nao cost** — o holding agregado `bank-bonds-aggregate` agora exibe o current value do SimpleFin como valor principal (`manualValue`), com o **cost** (`costBasis` = principal das transacoes) e o **gain/loss** no bloco de referencia expandido (invertendo o layout pre-v1.13.0, que mostrava cost como principal e market value como referencia). `applyFidelityBalanceUpdate(candidate, bondHoldings)` grava `bondMarketValues` (descKey->mv), `marketValueOverride` (soma) e `manualValue` (= soma). `applyBankBondsHolding` mantem `manualValue = marketValueOverride ?? principal` e `costBasis = principal` a cada mudanca de transacao. `pruneUnchangedBalanceCandidates` reaplica o candidate de Bank Bonds enquanto `bondMarketValues` estiver vazio no holding (garante que o mapa por-bond popule no primeiro sync pos-deploy mesmo se a soma nao mudou).
