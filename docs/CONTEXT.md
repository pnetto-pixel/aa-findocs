...
- **Bank Bonds: accrual estimado removido do Div TTM/YoC % (jul/2026, v1.14.1):** `Performance.jsx` mantem sua propria copia de `buildBondEvents` (agora reduzida a `buildBondEvents(bondIncome)`, so pagamentos reais), mesclada com o fetch de `/api/dividends`. Antes desta mudanca, essa era a copia **sem badge visivel** que inflava `Div TTM`/`YoC %`/`Total Div`/`totalReturn` do grafico silenciosamente — ver "Bank Bonds interest em todos os cards" na secao Dividends e Licoes Aprendidas.
...
- **Bank Bonds — Total Cost e Current Value consolidados com a tab Holdings (jul/2026, sha `400568e`, v1.8.0):** Total Cost agora vem de `computeBankBondsPrincipal` (modulo compartilhado `src/lib/bankBonds.js`, mesma funcao usada pelo holding agregado em `App.jsx`) — identico ao "valor principal" exibido em Holdings. Current Value le `marketValueOverride` da holding `bank-bonds-aggregate` quando disponivel (dado real do SimpleFin); sem override, `Current Value = Total Cost` (gain/loss zero — a antiga projecao de juros sintetica foi removida deste consumidor, mas continua alimentando a serie historica do Composition Evolution). Ver secao "Bank Bonds — Consolidacao Holdings vs Position Performance" acima para a causa raiz e o mecanismo de resolucao de ticker.
...
### Holdings manuais — Cash (auto-lock via SimpleFin — jul/2026, commit `62161f1`, v1.7.0)

O holding Cash (`CASH_ID = "cash-permanent"`) e sincronizado automaticamente via SimpleFin (Balance Updates aplicam sem revisao manual — ver "SimpleFin Feed — Auto-apply..."). Prop `valueLocked` em `ManualHoldingRow`, calculada como `h.id === CASH_ID && isAdmin`:
- Quando `valueLocked`, o form de edicao mostra "Synced automatically via SimpleFin" em vez de input de valor; `saveEdit()` pula a escrita de `manualValue`/`qty`/`price`.
- Target% permanece editavel normalmente.
- **Gate por admin, nao global:** usuario nao-admin (sem SimpleFin conectado) nunca tem Cash travado.
- Cash fica travado **independente da saude do sync** (erro, sem conexao, stale >48h) — decisao consciente do usuario, sem destrava condicional automatica. O heartbeat de Alerts avisa sobre problemas no sync, mas isso e so notificacao.
- Campo `simplefinSyncedAt` (gravado por `applyFidelityBalanceUpdate`) alimenta o bloco "Last Synced (SimpleFin)" no accordion expandido do holding, separado de `lastUpdated` (generico).
...
| Bank Bonds (CDs/bonds) | So pagamentos reais (Fidelity "INTEREST"/"CASH DIV") | Frontend-only; income real no campo `bondIncome` de `/api/transactions`. Accrual estimado removido de todo Income History/KPIs/cards em jul/2026 (merge `00988bc`, v1.14.1); o card "Bond Projections", ultima estimativa de juros de bond do app, foi removido logo em seguida (merge `038d6fa`, v1.14.2). **Nao existe mais nenhuma estimativa de juros de bond no app.**
...
- **Bank Bonds (CUSIP) nao passam por este endpoint** — income e calculado no frontend a partir so de pagamentos reais do campo `bondIncome` (item 36, PR #86 + #87; accrual estimado removido em jul/2026, merge `00988bc`, v1.14.1 — ver "Bank Bonds interest em todos os cards" abaixo)
...
- Retorna `{ events: [{ date, ticker, assetClass, incomeType: "dividend", amountPerShare, qtyHeld, totalReceived, currency: "USD", source: "api"|"fidelity" }], foreignTax: [{ date, ticker, assetClass, incomeType: "tax", totalReceived (negativo), currency: "USD", source: "fidelity" }], meta }` (campo `payDateUncertain` removido do shape em jul/2026 — ver fix abaixo)
...
- **Bug fix — dividendos fantasmas quando o pay date fica incerto (jul/2026, merge `77b4186`, v1.16.3, `v11 → v12`):** o fallback `payDateUncertain: true` (jul/2026, merge `d0054d4`, ver acima) continuava usando a ex-date como `date` do evento e o evento entrava normalmente em `events`/totais — o guard `if (date > todayISO) continue` nunca disparava porque a ex-date quase sempre ja passou (so a pay-date real, desconhecida, estaria no futuro). Confirmado linha a linha contra um extrato real da Fidelity (Accounts History CSV, jun/2026, conta Individual TOD): todo dividendo com pay date resolvido bateu centavo a centavo; 4 eventos com `payDateUncertain=true` (REXR $118.76, STAG $84.20, NKE $22.14, TSM $8.59, total $233.69) nao existiam no extrato porque o pagamento real cai num mes posterior (TSM e ADR e paga em outubro; os demais tinham ex-date em 30/06 ou 01/06 com pagamento real em julho). Fix: novo guard `if (payDateUncertain) { payDateUncertainSkipped++; continue; }` no loop principal — esses eventos sao excluidos de `events`/totais em vez de contados contra a ex-date. Novo `meta.payDateUncertainSkipped` para telemetria. Campo `payDateUncertain` removido do shape do evento (seria sempre `false` agora). Badge visual "EX-DATE" removido de `src/Dividends.jsx` (codigo morto, a condicao nunca mais e truthy). Efeito colateral positivo: evita um bug futuro de double-count — o de-dupe contra Fidelity e chaveado por `ticker|mes do pay date`; um evento fantasma registrado no mes da ex-date nao seria removido quando o CSV do mes certo importasse depois. **Nao investigado nesta sessao:** o valor por acao do STAG no evento fantasma parecia suspeito ($0.388 vs. ~$0.124/mes esperado) — pode ser bug de agregacao separado, mas como o evento inteiro foi excluido pela correcao, nao foi aprofundado (ver Pendentes no roadmap).
- Cache Redis versionado (`:dividends:v12:<txHash>`), TTL ate proximo fechamento do mercado US
...
- **Income History card** (mesmo design do "Portfolio Performance & Net Worth"): titulo + KPIs (All Time / YTD / This Month) **dentro** do card. Bar chart com views `Month | Quarter | Half | Year`.
  ...
  - **Bank Bonds interest em todos os cards — accrual estimado REMOVIDO (PR #86 + #87 + #88, item 36; bugfix jul/2026 — commit d93cd87, merge b96922a; remocao definitiva jul/2026 — merge `00988bc`, v1.14.1):** `buildBondEvents(transactions, bondIncome, todayISO)` no frontend gera eventos no **mesmo shape** dos dividendos de acoes, mas agora **so a partir de pagamentos reais** (`source: "fidelity"`, do campo `bondIncome` importado do Fidelity "INTEREST"/"CASH DIV"). O array `allEvents` (acoes + bonds) alimenta **todos** os cards: KPIs (All Time/YTD/This Month), bar chart, Position Dividends (YoC/Recovered de CUSIPs via cost basis), Dividend History e tabela Y/Y — todos passam a refletir **so dado real**; os valores CAEM para usuarios com Bank Bonds sem cobertura completa de pagamentos reais importados (comportamento desejado, decisao explicita do usuario). `bondLabel` virou a constante fixa `"bond interest"` (antes variava conforme havia mistura real/estimado). Badge "EST" e tooltip removidos do `DividendHistoryTable`. O guard `if (e.source !== "estimated")` no `useMemo kpis` (comparacao Y/Y) foi removido junto — nao tinha outra semantica alem de excluir estimados, que ja nao existem mais.
    - **O que foi deletado:** o step 3 (estimated accrual) de `buildBondEvents`, a funcao `accrueByFreqAsEvents`, e os helpers orfaos `minISO`/`addDaysISO` em `src/Dividends.jsx`. Em `src/Performance.jsx`, `buildBondEvents` foi reduzida ao "step 1" (so pagamentos reais) — assinatura `buildBondEvents(bondIncome)`, ja que `freqByCusip` nunca era consumido nesse arquivo; deletados como codigo morto `parseBondNotes`, `daysBetweenISO`, `minISO`, `freqFromDays`, `FREQ_DAYS`, `addDaysISO`, `accrueByFreqAsEvents`. **A assinatura de `buildBondEvents` agora DIFERE entre os dois arquivos** (`Dividends.jsx`: `(transactions, bondIncome, todayISO)`; `Performance.jsx`: `(bondIncome)`) — consequencia aceita da duplicacao por convencao do projeto (arquivos separados, ver Decisoes Tecnicas), coerente com o que cada call site de fato usa.
    - **Cards afetados em `Performance.jsx`:** `Div TTM`, `Total Div`, `YoC %` e o `totalReturn` do grafico de comparacao tambem paravam de contar accrual estimado — esse era o consumidor **sem nenhum badge visivel**, entao os numeros vinham inflados silenciosamente ali (ver Licoes Aprendidas).
    - **Historico do que existia antes (contexto para git blame):** entre PR #86/#87/#88 e o bugfix do commit d93cd87, o accrual coexistia com pagamentos reais — dimensionado pela frequencia calibrada (`freqByCusip[cusip]` -> `couponFreq` -> `"monthly"`), so emitido para periodos inteiramente decorridos, com `accrueFrom = ultimoPagamentoReal + 1 dia`. Essa mecanica inteira foi descontinuada nesta sessao; ficou preservada so no card Bond Projections (ver abaixo), que sempre foi forward-looking e sempre teve badge proprio.
    - **Nao tocados de proposito (v1.14.1):** `src/lib/bankBonds.js`, `src/AporteQuinzenal.jsx`, `api/dividends.js`, `src/Events.jsx`. Sem mudanca de backend, sem bump de cache Redis, sem migracao — os eventos estimados eram 100% client-side (recomputados a cada load), nunca persistidos.
...
- **Dividend History** (auditoria): tabela colapsavel com todo historico de pagamentos (Date · Ticker · $/Share · Qty · Total), ordenada por data desc, scroll vertical. Quarto card — apos "Dividends Monthly Y/Y".
  - **Badge "EX-DATE" — REMOVIDO (jul/2026, merge `77b4186`, v1.16.3):** existia desde `d0054d4`, exibido na coluna Ticker quando `e.payDateUncertain === true` — cor ambar `#e0a458`. Removido como codigo morto junto com o bug fix que passou a descartar esses eventos inteiramente em `api/dividends.js` (o campo `payDateUncertain` nunca mais chega truthy do backend) — ver "Bug fix — dividendos fantasmas quando o pay date fica incerto" acima. Antes disso o badge era a UNICA sinalizacao visual do caso; nenhum calculo/KPI a consumia, o que e justamente a raiz do bug (fallback silencioso pros agregados).
...
### Card Bond Projections — REMOVIDO (jul/2026, merge `038d6fa`, v1.14.2)

Card colapsavel que projetava cupons futuros de bonds em aberto (`buildBondProjections`, commit d6dc43b, estendido PR #107), com badge "EST" proprio. **Removido a pedido explicito do usuario**, logo apos a remocao do accrual estimado dos demais cards (v1.14.1): a decisao final foi que o app nao exibe nenhuma estimativa de juros de bond, nem retroativa nem forward-looking — so dado real vindo do SimpleFin (`bondIncome`) ou do import da Fidelity.

Removidos junto com o card (todos em `src/Dividends.jsx`, ninguem mais os consumia): `buildBondProjections`, o state `bondProjOpen`, o useMemo `bondProjections`, os helpers `parseBondNotes` / `daysBetweenISO` / `freqFromDays` / `FREQ_DAYS`, o step 2 de `buildBondEvents` (calibracao de `freqByCusip`, que existia so para alimentar as projecoes) e o `const todayISO`. `buildBondEvents` ficou com assinatura `buildBondEvents(bondIncome)`, agora identica a copia em `src/Performance.jsx`. Diff liquido: 388 linhas removidas, 10 adicionadas.

As notas historicas abaixo descrevem o card enquanto existia, preservadas para git blame (`FREQ_DAYS` module-level, frequencia calibrada via `freqByCusip`, intervalDays fixo por frequencia, valor `principal x couponPct/100 x intervalDays/365`) — nada disso existe mais no codigo.
...
- **Superseded (jul/2026, commit `62161f1`):** a secao "Balance Updates" (Approve/Dismiss por linha, descrita acima) foi removida — Cash e Bank Bonds agora sao aplicados automaticamente, sem revisao manual. Ver "SimpleFin Feed — Auto-apply de Cash e Bank Bonds" mais abaixo.
...
- Fora do escopo (permanece pendente): resolucao de CUSIP individual por bond (feed SimpleFin nao traz CUSIP — reconciliacao agregada e decisao de design permanente, nao uma limitacao temporaria); Fase 4 (multi-usuario / claim de setup token).

-----

### SimpleFin Feed — Fidelity automation, Fase 3: hardening + limpeza (jul/2026, commit `fbc9a15`, merged em main)

Ultima fase planejada antes da Fase 4 (multi-usuario, ainda pendente). Fecha o plano [`docs/plans/simplefin-fidelity-feed.md`](./plans/simplefin-fidelity-feed.md) (ver §6).

- **Heartbeat de sync no painel de Alerts (Bell):** nova funcao `refreshSimplefinHeartbeat()` em `src/App.jsx`, admin-only (reusa `isAdmin`, ja existente via `useMemo`), chamada no load effect logo apos `refreshAlerts(txs, loadedBondIncome)`. Busca `GET /api/fidelity-pending?resource=status`, cujo shape real e `{ ok, connected, lastSync, lastError, nextSyncAt }` (sem `simplefinErrors`). Threshold: dispara alerta se `lastError` esta presente na ultima tentativa, OU se `lastSync` e nulo/mais antigo que 48h. Tres mensagens possiveis: erro real (`"SimpleFin: falha ao sincronizar — <erro>"`), nunca sincronizado (`"SimpleFin: nunca sincronizado"`), ou stale com contagem de dias (`"SimpleFin: sem sincronizar ha N dia(s)"`). Reusa o padrao `mergeAlerts`/`setAlertLog` ja existente (mesmo usado por split, dividendo, earnings, bond maturity) — novo `type: "simplefin_heartbeat"`, sem componente de UI novo. **Diferenca importante:** o `id` e estavel (`"simplefin_heartbeat"`, sem data embutida), entao quando o sync volta a ficar saudavel o alerta precisa de remocao **explicita** via `setAlertLog(prev => prev.filter(a => a.id !== HEARTBEAT_ID))` — os outros tipos de alerta "somem" implicitamente porque o id embute a data e nunca mais bate.
- **Remocao do caminho scraper obsoleto (nunca ativado em producao):** deletado `api/ingest-fidelity.js` — libera 1 slot de Serverless Function do Vercel Hobby (de 12/12 para 11/12). Deletado o diretorio `docs/plans/scraper/` inteiro (README.md, parse-fidelity.mjs, scrape.mjs, package.json, workflow-sync.yml). `api/fidelity-pending.js`: comentario de cabecalho atualizado — nao referencia mais `ingest-fidelity.js` como endpoint companheiro ativo; documenta que o staging e populado so via `?resource=sync`. Confirmado por grep full-repo: nenhuma referencia restante a `ingest-fidelity` em `api/`, `lib/`, `src/`, `vercel.json` — so em docs historicos (`docs/plans/item-38-fidelity-export-automation.md` e `docs/plans/item-38-activation-runbook.md`, mantidos intencionalmente por documentarem decisao historica). `constantTimeEqual` (usado por `ingest-fidelity.js`) ja vivia em `lib/auth.js` desde o hardening batch — nada ficou orfao.
- `package.json` `1.3.1 -> 1.4.0` (minor). Build (`npm run build`, vite, 2309 modulos) e suite completa `test/*.test.mjs` (70 casos) verdes.
- Pendente: Fase 4 (multi-usuario / claim de setup token) — ver `docs/plans/simplefin-fidelity-feed.md` §6.

-----

### SimpleFin Feed — Auto-apply de Cash e Bank Bonds + Cash read-only em Holdings (jul/2026, commit `62161f1`, merged em main, v1.7.0)

Extensao pontual sobre as Fases 1-3 (nao uma nova fase do plano `docs/plans/simplefin-fidelity-feed.md`): decisao explicita do usuario de abrir mao da revisao manual (Approve/Dismiss) especificamente para os dois Balance Updates do SimpleFin — Cash e Bank Bonds. Trades e Income do card "Fidelity Import" continuam 100% manuais, intocados.

- **Auto-apply em `src/Transactions.jsx`:** os dois pontos que antes so computavam `freshBalance` e enfileiravam para aprovacao (`setPendingBalance`) — effect de load on-mount e dentro de `runFidelitySync()` (disparado pelo botao "Sync Fidelity") — agora chamam `onApproveFidelityBalance?.(c)` automaticamente para cada candidate fresco, e sempre limpam `balanceCandidates` no staging remoto (`patchPendingFidelity`). Secao "Balance Updates" (Approve/Dismiss por linha) removida inteiramente do card "Fidelity Import" — nao sobra nada para aprovar. Codigo morto removido junto: `approveBalanceCandidate`, `dismissBalanceCandidate`, estado `pendingBalance`/`balanceActionId`, prop `onDismissFidelityBalance`/`fidelityBalanceActionInFlight`. Badges de contagem do card "Sync & Automation" ajustados.
- **Protecao contra double-apply:** novo `appliedBalanceIdsRef` (`useRef(new Set())`), chave composta `` `${candidate.id}:${candidate.proposed}` `` — nao so `candidate.id`. O `id` e deterministico por conta+tipo (`lib/simplefin-map.js`/`computeBalanceCandidates`) e nunca muda entre syncs; so a chave composta com o valor proposto permite reaplicar quando o saldo real muda, e ainda bloqueia reprocessamento do mesmo valor numa janela de corrida antes do `holdings` prop atualizar.
- **Fix em `pruneUnchangedBalanceCandidates`:** candidate de Bank Bonds agora compara contra `holding.marketValueOverride` (era `manualValue`, campo errado — `manualValue` de Bank Bonds e derivado das transacoes, nao o valor de mercado).
- **`src/App.jsx`:** novo campo `simplefinSyncedAt`, gravado por `applyFidelityBalanceUpdate` (branch `kind === "cash"`) — timestamp dedicado do ultimo sync bem-sucedido, separado de `lastUpdated` (generico, bumpado por qualquer edicao inclusive Target%). Novo bloco "Last Synced (SimpleFin)" no accordion expandido (`driftOpen`) do holding Cash, espelhando o bloco ja existente de Bank Bonds (`marketValueOverrideAsOf`). Nova prop `valueLocked` em `ManualHoldingRow`, calculada como `h.id === CASH_ID && isAdmin` — gate por admin, nao global: usuario nao-admin (sem SimpleFin conectado) nunca tem Cash travado, senao perderia a unica forma de manter o saldo atualizado. Quando `valueLocked`, o form de edicao mostra "Synced automatically via SimpleFin" em vez de input, e `saveEdit()` pula a escrita de `manualValue`/`qty`/`price` (guard `!isBankBonds && !valueLocked`) — Target% continua editavel normalmente. Bank Bonds **nao mudou de comportamento** — ja era read-only desde o PR #93 (`isBankBonds` guard), independente da nova prop `valueLocked`.
- **Decisao de design (usuario, consciente):** Cash fica sempre travado para o admin na Tab Holdings, independente da saude do sync SimpleFin (erro, sem conexao configurada, ou stale >48h) — sem destrava condicional automatica. Se o sync quebrar por dias, a correcao do valor de Cash exigiria intervencao fora do app (Redis direto/chat), nao pelo app. Trade-off aceito conscientemente em troca de simplicidade. O heartbeat de Alerts (`refreshSimplefinHeartbeat`) ja avisa sobre erro/staleness do sync, mas isso e so notificacao — nao desbloqueia o campo.
- Backend `api/fidelity-pending.js` intocado; sem bump de `CACHE_VERSION`; sem mudanca de shape no Redis `:fidelity-pending`. `package.json` `1.6.0 -> 1.7.0` (minor).
- **Auditoria (2 rodadas):** rodada 1 reprovou por bug bloqueante — `appliedBalanceIdsRef` usava so `c.id` como chave (sem o valor), travando o candidate permanentemente apos a primeira aplicacao; atualizacoes reais de saldo em syncs seguintes eram descartadas silenciosamente apesar do toast de sync reportar sucesso. Fix (chave composta `id:proposed`) aplicado e aprovado na rodada 2. Merge direto em `main` via `git merge --no-ff`, sem PR — commit `62161f1`.
- Pendente: Fase 4 (multi-usuario / claim de setup token).
...
### Bank Bonds — Consolidacao Holdings vs Position Performance (jul/2026, sha `400568e`, merged em main, v1.8.0)

Fecha o "ponto cego" #1 levantado na sessao anterior (ver acima): o usuario via 4 numeros divergentes para o mesmo holding Bank Bonds (Total Cost e Current Value diferentes entre a tab Holdings e o card Position Performance). Objetivo: 2 valores canonicos — **Custo** (das transacoes, identico nas duas telas) e **Current Value** (do SimpleFin quando disponivel, senao igual ao custo).

- **Causa raiz:** duas implementacoes divergentes do mesmo replay de transacoes Bank Bonds coexistiam — `computeBankBondsPrincipal` (ticker-agnostica, `App.jsx`) e `computeBankBondsValueAt` (agrupada por ticker/CUSIP, `Performance.jsx`). Quando um bond vencia, a redemption vinda do SimpleFin usava o texto da `description` como pseudo-ticker (o feed nao traz CUSIP) — se esse texto nao batia com o ticker da compra original, o agrupamento por ticker criava uma posicao orfa de venda que nunca baixava o custo da compra real, inflando o Total Cost agregado no Performance. O "Current Value" do Performance tambem nunca usava o dado real do SimpleFin — era uma projecao sintetica (custo + juros lineares a partir de coupon%/maturity extraidos de `notes`).
- **Novo modulo compartilhado `src/lib/bankBonds.js`:** move `computeBankBondsPrincipal` (de `App.jsx`) e `computeBankBondsValueAt` (de `Performance.jsx`) para uma unica fonte de verdade. `computeBankBondsValueAt` ganhou uma pre-passagem de resolucao de ticker: para redemptions do SimpleFin (`tx.redemption === true`) cujo ticker bruto nao bate com nenhuma compra conhecida, tenta resolver via `extractBondMeta` (`src/lib/parsing.js`, ja usado pelo import CSV) comparando `descKey` (issuer+coupon%+maturity) contra as compras Bank Bonds que tem `couponRate`/`maturityDate`/`shortName` preenchidos — mesmo padrao ja usado por `src/Transactions.jsx` para resolver CUSIP faltante no CSV. Corrige o matching retroativamente para bonds importados via CSV; bonds inseridos manualmente (sem esses campos) continuam sem match — comportamento antigo preservado, degrada sem quebrar.
- Novo helper `listBankBondsTickers` — evita que o grafico Composition Evolution por Ticker perca a linha "decaindo a zero" de um bond ja resgatado.
- **`src/Performance.jsx`, `positionRows`:** Current Value de Bank Bonds agora le `marketValueOverride` da holding `bank-bonds-aggregate` (via prop `holdings`, ja recebida) em vez da projecao de juros. Sem override disponivel: `Current Value = Total Cost` (gain/loss zero — decisao explicita do usuario). Com override: distribuicao proporcional do valor real do SimpleFin entre os tickers Bank Bonds visiveis, por peso de `totalCost`.
- **Safety-net:** `console.warn` se a soma pos-fix divergir do valor canonico (`computeBankBondsPrincipal`) por mais de 1 centavo — sinaliza redemptions nao resolvidas sem tentar uma segunda correcao automatica.
- A projecao de juros linear foi mantida INTACTA dentro de `computeBankBondsValueAt` — ainda e a unica fonte de dado para a serie historica do grafico Composition Evolution (nao ha snapshot real do SimpleFin para datas passadas). So o consumidor Position Performance parou de usa-la.
- `BANK_BONDS_ID` (`"bank-bonds-aggregate"`) ficou duplicado como string constante em `App.jsx` e `Performance.jsx` (comentado, para evitar import circular — `Performance.jsx` e lazy-loaded a partir de `App.jsx`) — decisao consciente do auditor, baixo risco.
- Novo `test/bank-bonds.test.mjs` (5 testes): match exato, resolucao via `descKey`, redemption orfa sem match, `computeBankBondsPrincipal` como referencia canonica.
- 100% client-side — sem mudanca em `api/fidelity-pending.js`, sem bump de cache de `perf-history`.

**Limitacao conhecida, documentada explicitamente:** a resolucao de ticker so funciona para bonds importados via CSV Fidelity (unicos com `couponRate`/`maturityDate`/`shortName` gravados). Bonds inseridos manualmente continuam sem matching individual — o SimpleFin nao retorna CUSIP, decisao de design permanente ja documentada (Fase 2 do SimpleFin Feed). **Nao validado empiricamente:** se a `description` do SimpleFin e byte-identica a "Symbol Description" do CSV Fidelity para o mesmo bond — forte probabilidade (mesma instituicao, mesmo texto fonte), mas sem confirmacao por par real observado ainda. Validar apos o primeiro sync real pos-deploy: Holdings "valor principal" deve bater com Position Performance "Total Cost" (linha resumo Bank Bonds), e "Market Value (SimpleFin)" deve bater com "Current Value" (mesma linha).

-----

### SimpleFin Feed — Bank Bonds "sumindo" do fluxo: diagnostico + fixes (jul/2026, sha `fe72fbe`, merged em main, v1.8.1)

Item 3 da sessao de continuacao SimpleFin (escopo: "matching de bonds — alguns nao estao retornando"). Uma investigacao dentro da mesma sessao levantou 4 causas tecnicas candidatas para Bank Bonds "sumirem"/nao aparecerem no fluxo SimpleFin; esta entrega endereca 2 delas como bugs reais, resolve 1 como falso-alarme (so copy), e deixa 1 documentada como decisao de produto (nao mexida).

**Diagnostico das 4 causas:**

1. **Falso alarme, resolvido so via copy:** compra de bond nunca vira transacao automatica do SimpleFin (o feed nao estrutura trades) — mas isso ja cai corretamente no bucket `unmapped` existente (nunca era descartado silenciosamente). O problema era so falta de comunicacao clara. Fix: copy da secao "Unmapped — needs review" em `src/Transactions.jsx` agora menciona explicitamente compra de bond/CD novo como um dos motivos de uma linha aparecer ali.
2. **Decisao de produto, nao implementada:** janela fixa de 44 dias do sync (`SIMPLEFIN_WINDOW_DAYS`, `api/fidelity-pending.js`) — eventos (interest/redemption) mais antigos que isso nunca sao buscados. Aumentar tem trade-off (mais chamadas a Bridge do SimpleFin, risco de acionar advisory de cap). Fica documentada como limitacao conhecida, nao mexida.
3. **Bug real, corrigido:** `computeBalanceCandidates` (`lib/simplefin-map.js`) tinha dois filtros silenciosos no loop de holdings ao somar Bank Bonds — um pra excluir acoes/ETFs (`symbol !== ""`, comportamento correto, mantido intacto) e outro pra excluir `market_value` invalido (`!isFinite(mv)`, esse SIM era um bug — holding de bond valido podia sumir da soma sem nenhum sinal). Fix: `computeBalanceCandidates` agora retorna `{ candidates, skipped }`; holdings com `market_value` invalido entram em `skipped` (mesmo shape de `unmappedItem`) e sao mesclados no array `unmapped` ja exposto na UI — aparecem em "Unmapped — needs review" em vez de desaparecer silenciosamente.
4. **Bug real, corrigido (o mais arriscado dos quatro):** `applyFidelityBalanceUpdate` (`src/App.jsx`), branch `kind === "bank-bonds"`, descartava o update do SimpleFin com um toast de erro invisivel (roda em background, sem o usuario olhando) quando a holding agregada `bank-bonds-aggregate` ainda nao existia (so e criada hoje quando o usuario cadastra a primeira transacao Bank Bonds manualmente). Ou seja: usuario conecta o SimpleFin, tem bonds na Fidelity, mas nunca cadastrou nenhuma transacao Bank Bonds ainda -> o valor do SimpleFin era calculado e descartado, sem holding, sem alerta.

**Mudanca tecnica do fix da causa 4 (a parte mais delicada):**

- `applyFidelityBalanceUpdate` agora AUTO-CRIA a holding `bank-bonds-aggregate` quando ela nao existe, com `manualValue: candidate.proposed` (precisa ser o valor real, nao 0 — um filtro existente esconde holdings com valor zero do Dashboard, ver "Auto-hide de holdings zerados"), `marketValueOverride`/`marketValueOverrideAsOf`, e um novo campo `derivedFromTransactions: false`.
- Sem esse novo campo, o holding recem-criado seria zerado de volta no proximo load da pagina: `applyBankBondsHolding` roda em todo load/refresh e recalcula `manualValue` a partir de `computeBankBondsPrincipal(transactions)` — que e sempre 0 sem nenhuma transacao Bank Bonds real. Fix: novo guard em `applyBankBondsHolding` — se a holding existe, nao ha transacao Bank Bonds ainda (`hasBankBondTx === false`), e `derivedFromTransactions === false` (checagem estrita, nao afeta holdings antigas com o campo `undefined` — essas continuam reconciliando normalmente), a funcao retorna `null` (nao reconcilia, nao sobrescreve).
- Quando o usuario finalmente cadastra a primeira transacao Bank Bonds real (`hasBankBondTx` vira `true`), o holding e "graduado": o patch normal passa a setar `derivedFromTransactions: true` tambem, inclusive num caso-limite (`needsGraduation`) onde o `manualValue` ja coincidia com o `principal` recem-computado no momento exato da graduacao — sem esse tratamento a holding ficaria presa em `derivedFromTransactions: false` para sempre nesse cenario especifico.
- **Comportamento resultante documentado (nao e bug):** depois de auto-criada, syncs subsequentes do SimpleFin so atualizam `marketValueOverride`/`marketValueOverrideAsOf` (a referencia de mercado) — o `manualValue`/"Total Cost" fica congelado no valor do primeiro sync ate o usuario lancar a transacao real de compra, que e quando a reconciliacao por transacoes assume de vez.
- Testado via 2 novos casos em `test/simplefin-map.test.mjs` (a mudanca em `App.jsx` nao e testavel via harness — validada por leitura/rastreamento manual de codigo pelo coder e revisao linha a linha pelo auditor, confirmando que os 3 call sites de `applyBankBondsHolding` tratam retorno `null` como "nada mudou", sem risco de apagar holdings do usuario).
- 100% client-side (exceto o pequeno ajuste de shape de retorno em `lib/simplefin-map.js`) — sem mudanca em endpoints, sem bump de cache.

-----

### SimpleFin Feed — Resolucao de ticker em transacoes de INTEREST (jul/2026, sha `747fcc2`, merged em main, v1.9.0)
...
1. **Auto-resolucao no backend**, cruzando o issuer do interest contra os HOLDINGS retornados pelo SimpleFin no mesmo sync (que tem description COMPLETA, com coupon%+maturity — confirmado em fixture de teste). Quando exatamente 1 holding da conta bate com o issuer (apos normalizacao de espacos/caixa), e esse holding resolve para um CUSIP conhecido do usuario (via `descKey`, mesma tecnica ja usada pra redemption), o ticker do interest e sobrescrito com o CUSIP real automaticamente. Se o match e unico mas o usuario ainda nao tem essa compra cadastrada (sem CUSIP conhecido), o ticker fica como o issuer (status quo) mas o evento carrega um campo `descKey` como dica. Se ha 0 ou 2+ matches (ambiguo — ex: duas contas com bonds do mesmo emissor), nada e resolvido automaticamente — comportamento estritamente conservador, nunca resolve por aproximacao.
2. **Dropdown editavel no card "Fidelity Income"** (`src/Transactions.jsx`), pra quando a resolucao automatica falha/fica ambigua — usuario escolhe manualmente entre os CUSIPs de Bank Bonds ja conhecidos (dropdown, nao texto livre, pra evitar erro de digitacao) antes de aprovar. A escolha e aplicada em `approvePendingFidBond()` antes do calculo de dedupe e do push final.

**Mudanca tecnica:**

- **Novo modulo compartilhado `lib/bond-meta.js`** (raiz do repo, sem dependencias de frontend/papaparse, importavel tanto do client quanto de funcoes serverless): `extractBondMeta` (movida de `src/lib/parsing.js`, que agora importa e re-exporta com o mesmo nome — zero mudanca de contrato) + `buildKnownBondsByDescKey(transactions)` (nova, consolida 2 implementacoes que ja existiam duplicadas: uma privada em `src/lib/bankBonds.js`, outra inline em `src/Transactions.jsx`'s `ImportModal`).
- `lib/simplefin-map.js`: novo helper `bankBondHoldings(account)` (extrai o filtro `symbol === "" && description !== "CASH"` que ja existia inline em `computeBalanceCandidates`, agora reaproveitado tambem pela logica de interest); `mapOneTransaction`/`mapSimplefinPayload` ganham um parametro opcional `knownBondsByDescKey` (default Map vazio — comportamento identico ao anterior se omitido).
- `api/fidelity-pending.js`: reordena a leitura de `live`/`liveTx` para antes da chamada de `mapSimplefinPayload`, construindo `knownBondsByDescKey` a partir das transacoes ja salvas do usuario e passando pro mapper.
- `src/Transactions.jsx`: novo `knownBankBondTickers` (Set de CUSIPs conhecidos) e state `bondTickerEdits`; dropdown na tabela Fidelity Income so para linhas `kind === "interest"` (dividend/tax continuam texto puro, ja resolvem corretamente); `approvePendingFidBond()` aplica o override escolhido antes de dedupe/push, limpa o state apos aprovacao.
- **Confirmado que nenhuma mudanca foi necessaria em `src/Dividends.jsx`/`src/Performance.jsx`** — esses consumidores ja indexam por igualdade exata de string contra o `ticker`; como o ticker chega resolvido (CUSIP real) ou e corrigido manualmente antes da aprovacao, eles ja casam corretamente sem qualquer alteracao.
- 5 novos testes em `test/simplefin-map.test.mjs`: match unico + CUSIP conhecido resolve; match unico sem CUSIP conhecido (descKey vira hint); 0 matches; 2+ matches (ambiguo); normalizacao de espacos/caixa. Suite completa: 83/83 passando.
- **Nota de processo:** a implementacao foi interrompida no meio por um limite de sessao da API (Claude Code), retomada pela sessao principal que corrigiu 1 bug real encontrado num teste (chave `descKey` esperada no teste de normalizacao tinha um trecho " CD" a mais, inconsistente com o que `extractBondMeta` realmente extrai da description do holding usado no mesmo teste) antes de mandar para auditoria.

**Fora de escopo (decisao ja tomada):**

- Correcao retroativa de `bondIncome` ja aprovado no passado com ticker errado (dado ja persistido) — nao foi feita nesta rodada.
- Edicao de ticker para linhas `dividend`/`tax` no card Fidelity Income — ja resolvem corretamente hoje, fora de escopo.
...
**Hotfix critico (jul/2026, sha `8255619`, v1.10.1):** o merge da Fatia 1 (v1.10.0) quebrou a tab Transactions inteira — tela preta ao clicar, reportado pelo usuario minutos apos o deploy. Causa: `unboundBondHoldings` (`useMemo`, `src/Transactions.jsx`) referenciava `pendingBondHoldings`/`bondBindings` (ambos `useState`) declarados MAIS ABAIXO no corpo do componente — violacao de temporal dead zone (`const`/`let` no mesmo escopo de funcao nao sao utilizaveis antes da propria linha de declaracao), gerando `ReferenceError` em todo render de `TransactionsView`. Sem error boundary pra conter, o React desmonta a arvore inteira — tela preta. **Passou pela auditoria porque o ambiente de implementacao nao tem browser** (`npm run build`/testes de logica nao pegam erro de ordem de execucao em runtime — TDZ e sintaticamente valido, so quebra quando o codigo roda). Fix: mover as duas declaracoes de `useState` pra antes do `useMemo` que as consome. Corrigido fora do pipeline de 4 agentes (planner/coder/auditor/docs) — dado producao quebrada, a sessao principal diagnosticou e aplicou o fix diretamente, validou build (verde) + suite completa (90/90) antes do push direto pra `main`.

**Hotfix de persistencia (jul/2026, sha `8fc95b5`, v1.10.2):** usuario reportou que binds manuais confirmados na tela "Bond Matching" voltavam a pedir selecao apos um refresh. Diagnostico: `confirmBondMatch` fazia update OTIMISTA do estado local (marcava vinculado na hora) enquanto `patchPendingFidelity` engolia qualquer erro (`catch {}`) e nunca checava `res.ok` — uma gravacao que falhasse no servidor parecia confirmada ate o proximo refresh. Fix: `patchPendingFidelity` passou a retornar `res.ok` (nao-throwing, callers fire-and-forget ignoram o retorno); `confirmBondMatch` RELÊ `bondBindings`/`bondHoldings` do servidor apos o PATCH e so remove a linha da lista pendente quando o bind de fato persistiu, com mensagem de erro visivel se a gravacao falhar ou nao persistir. Licao: **UI otimista que nao verifica a gravacao mente pro usuario** — para acoes que precisam persistir, checar `res.ok` e/ou reler o estado real do servidor, especialmente quando o helper de escrita e fire-and-forget silencioso.

### SimpleFin Feed — Matching de Bank Bonds, Fatia 2 de 3: backfill de metadata (jul/2026, sha `faca800`, merged em main, v1.11.0)

Continuacao direta da Fatia 1. Quando existe um bind confirmado (`descKey -> CUSIP`) na tela "Bond Matching", preenche automaticamente `couponRate`/`maturityDate`/`shortName` E o campo de texto `notes` nas transacoes Bank Bonds do usuario com aquele CUSIP, a partir da description do holding do SimpleFin. Resolve o caso real do usuario: 7 dos seus 11 bonds nao tinham cupom/vencimento salvos, e depois do bind manual ele queria que os notes atualizassem como os outros 4.

- **Regras (confirmadas com o usuario):** nunca sobrescreve, so preenche campo vazio/null; roda **retroativo e automatico** (cobre binds ja confirmados antes desta fatia, sem botao); preenche **`notes` tambem** porque `computeBankBondsValueAt` (`src/lib/bankBonds.js`) le o accrual de juros de `tx.notes` via regex — sem `notes`, os 7 bonds ficariam "achatados" (sem projecao de juros) no grafico Composition Evolution. Formato `"4.20% | 07/08/2030"` (mesmo dos bonds existentes, casa o regex de accrual).
- **`lib/bond-meta.js`:** nova funcao pura `backfillBondMetadata(transactions, cusip, holdingDescription)` — usa `extractBondMeta` pra extrair a metadata; preenche so campos vazios em toda tx Bank Bonds com ticker == cusip (buy e sell); **retorna a mesma referencia de array quando nada muda** (base da deteccao de mudanca e da prevencao de loop de efeito).
- **`src/Transactions.jsx`:** novo `useEffect` de reconciliacao sobre todas as entradas de `bondBindings` (deps incluem `transactions`/`bondBindings`/`pendingBondHoldings`), chama `persist(next)` so se `next !== transactions`. Cobre binds novos e retroativos num unico codigo. Guards anti-loop: deteccao por identidade de referencia + `backfillContentSigRef` (assinatura de conteudo) + `backfillInFlightRef`. Auditor validou manualmente que o ciclo converge em 2 disparos / 1 unico PUT ao servidor — critico porque build+testes nao pegam loop de efeito React sem browser (mesma licao do hotfix de TDZ v1.10.1).
- **Limitacao conhecida:** so funciona pra holdings cuja description do SimpleFin parseia via `extractBondMeta` (formato `EMISSOR CD X.XXXXX% MM/DD/YYYY`); Treasuries/formatos diferentes ficam no-op (comportamento correto, nao bug). Sem mudanca em `handleSync` (mantem a garantia de nunca modificar `:transactions`), sem efeito em `computeBankBondsPrincipal`/calculo de valor. 7 novos testes em `test/bond-meta.test.mjs`; suite completa 97/97.
...
Continuacao da linha de trabalho de Bank Bonds SimpleFin (Fatias 1 e 2 + hotfixes acima). Nem o import CSV da Fidelity nem o sync do SimpleFin trazem mais CUSIP (a Fidelity parou de incluir CUSIP no Accounts History recentemente) — em vez de exigir que o usuario digite CUSIP manualmente, o app gera um identificador sintetico determinístico derivado de cupom+vencimento.

**Formato do ID** (decidido com o usuario): `<YYYYMMDD><cupom em pontos-base>`, sem prefixo, sem separador. Ex: cupom 3,95% vencendo 2029-05-08 -> `20290508395`; cupom 3,80% mesmo vencimento -> `20290508380`. O vencimento tem sempre 8 digitos fixos (YYYYMMDD), o que garante que nao ha colisao por concatenacao (os 8 primeiros chars sao sempre o vencimento, o resto e o cupom). O cupom era obrigatorio no ID porque so o vencimento colidiria nos dados reais do usuario (dois bonds vencendo em 05/08/2029). Cupons quebrados (ex: 4,125%) usam `x1000` como fallback determinístico (`202905084125`), sem ponto decimal.

**Por que isso funciona / o que destrava:** a resolucao de INTEREST/REDEMPTION do SimpleFin (mecanismos das v1.9.0/v1.10.0) depende so de `couponRate`/`maturityDate`/`shortName` estarem na transacao, NAO do formato do `ticker` (e ticker-agnostica). Bonds cadastrados com ID sintetico **auto-resolvem** contra eventos futuros do SimpleFin sem precisar da tela "Bond Matching". O CUSIP real deixa de ser necessario como identidade.

**Mudanca tecnica:**

- **`lib/bond-meta.js`** — novo helper puro `generateSyntheticBondTicker(couponRate, maturityDate)`: encoding `Math.round(couponRate*100)` (basis points) quando inteiro, fallback `x1000` pra cupom quebrado; valida `maturityDate` (`YYYY-MM-DD` + data real); retorna falsy se inputs invalidos.
- **`src/lib/parsing.js` (`parseFidelityCSV`)** — linha de compra Bank Bond sem CUSIP com description parseavel (cupom+vencimento) e sem match em `knownBondsByDescKey` -> gera o ticker sintetico automaticamente (zero digitacao — a metadata ja vem do CSV), em vez de marcar `needsTicker`. Como o ticker sintetico e so-digitos e NAO casa `CUSIP_RX` (9 chars) nem `inferAssetClass`, forca `assetClass="Bank Bonds"` explicitamente (senao cairia em "Stocks").
- **`src/Transactions.jsx` (`TransactionForm`)** — link opt-in "Don't have a CUSIP? Generate ID from coupon + maturity" (so quando `assetClass==="Bank Bonds"` e ticker vazio) que expande mini-form (Coupon %, Maturity, Issuer/Short Name opcional) + botao Generate (funcao pura por clique, sem `useEffect`). `handleSubmit` passou a incluir `couponRate`/`maturityDate`/`shortName`/`notes` no tx (antes o form de criacao nunca incluia esses campos — so o parser CSV e a edicao inline). O ticker gerado fica editavel.

**Correcao de 2 bugs bloqueantes na auditoria (rodada 1 reprovou, rodada 2 aprovou):** os guards de REDEMPTION (`src/lib/parsing.js`) e de INTEREST exigiam que o ticker recuperado casasse `CUSIP_RX` (9 chars). Quando um bond com ticker sintetico (11 chars) recebe, num reimport de CSV, uma linha REDEMPTION/INTEREST com Symbol em branco (comportamento padrao da Fidelity), o symbol e recuperado via `knownBondsByDescKey` como o ticker sintetico — e era descartado silenciosamente pelo `CUSIP_RX`. Resultado seria: juros nunca contabilizados, posicao nunca zerada no vencimento — quebrando exatamente o caso de uso da feature. **Fix:** flag `symbolFromHistory`/`symbolIFromHistory` setada so quando o symbol vem do lookup por descKey; os guards passam a aceitar ticker recuperado do historico (CUSIP real OU sintetico) mas ainda rejeitam Symbol cru invalido. Tambem removido um filtro redundante de `CUSIP_RX` no alerta de vencimento de bond (`src/App.jsx` `refreshAlerts`) que silenciaria o alerta "vence em N dias" pra bonds com ID sintetico (`assetClass="Bank Bonds"` + `maturityDate` ja basta). 4 testes novos em `test/fidelity-parser.test.mjs` cobrindo os 2 repros + 2 de nao-regressao.

**Limitacoes conhecidas:**

- Nao migra os 11 bonds legados (decisao do usuario: continuam com CUSIP real + tela Bond Matching).
- Colisao de ticker aceita como limitacao: dois bonds com mesmo cupom+vencimento e emissores diferentes gerariam o mesmo ticker sintetico (o emissor e capturado em `shortName` pra o match com SimpleFin, mas nao entra no ticker). Nao existe nos dados atuais do usuario; ticker e editavel como escape manual.
- So funciona pra bonds cuja description parseia via `extractBondMeta`; caso residual (Treasury em formato diferente) continua caindo no comportamento pre-existente (drop/entrada manual).
- Gerador nao existe na edicao inline da tabela (so no form de criacao e no CSV).

-----

### Bank Bonds — sync cria compras + current value por bond do SimpleFin (jul/2026, v1.13.0)

Quatro mudancas pedidas pelo usuario, fechando o ciclo de Bank Bonds/SimpleFin. A "tela Bond Matching" foi **removida** — nao e mais necessaria porque o sync agora cria as transacoes de compra direto e a resolucao e automatica por `descKey`.

**(1) Sync do SimpleFin cria compras de bonds novos** — terceira forma de entrada, junto de manual + CSV import. `lib/simplefin-map.js` ganhou `bondBuyTransactions(account, knownBondsByDescKey)`: pra cada holding de bond do snapshot cujo `descKey` (cupom+vencimento+emissor) **nao** esta em `knownBondsByDescKey` (bond que o usuario ainda nao tem), gera uma transacao `buy` Bank Bonds. Ticker = ID sintetico (`generateSyntheticBondTicker`). **Custo = `purchase_price x shares`** (o feed do SimpleFin traz esses campos — correcao do usuario sobre a suposicao inicial de que o feed nao tinha custo), com `cost_basis` como alternativa ja multiplicada e o `market_value` como ultimo fallback (`bondCostBasis(h)`). Representado a par: `price=1000`, `qty = cost/1000`, entao `qty*price === cost` (consistente com a representacao de Bank Bonds do import CSV). O gain/loss real cai naturalmente de (market value por bond − esse custo); so o fallback pra market value comeca o gain/loss em ~0. `simplefinId` estavel (`sfbond-buy:<descKey>`) pro dedup server-side. As compras vao pra `pendingFid` (staging normal) — o usuario aprova como qualquer trade sincronizado; nada e escrito silenciosamente. Bonds ja possuidos (descKey conhecido) nunca sao recriados; bonds cuja description nao parseia sao pulados (nao chuta).

**(2) Card "Bond Matching" removido** (`src/Transactions.jsx`) — junto de `confirmBondMatch`, `unboundBondHoldings`/`boundBondHoldingsCount`, e o estado de UI (`bondMatchingOpen`/`bondMatchPicks`/`bondMatchConfirmingKey`). Mantidos: `pendingBondHoldings` (carrega os market values por bond, threaded pro approve) e `bondBindings` (ainda alimenta o backfill de metadata; agora populado so pelo auto-bind server-side).

**(3) Position Performance casa o current value individualmente por bond** — novo helper puro `computeBankBondsMarketValue(transactions, bondMarketValuesByDescKey)` em `src/lib/bankBonds.js`: agrupa por ticker resolvido, deriva o `descKey` de cada posicao a partir do `couponRate`/`maturityDate`/`shortName` da compra, e usa o market value que o SimpleFin reportou pra aquele bond (`bondMarketValues[descKey]`). Bond que o SimpleFin **nao** retornou cai pro proprio custo (gain/loss 0, `marketValueSource: "cost"`) — nunca some do total, so nao mostra ganho. Substitui a distribuicao proporcional-por-custo do `marketValueOverride` agregado (v1.8.0). `positionRows` (`src/Performance.jsx`) passou a ler desse helper.

**(4) Holdings mostra current value, nao cost** — o holding agregado `bank-bonds-aggregate` agora exibe o current value do SimpleFin como valor principal (`manualValue`), com o **cost** (`costBasis`) no bloco de referencia expandido e o **gain/loss (market value − cost)** ao lado do valor principal (na linha 1, estilo day-change dos tickers auto — v1.13.1). `applyFidelityBalanceUpdate(candidate, bondHoldings)` grava `bondMarketValues` (descKey->mv), `marketValueOverride` (soma) e `manualValue` (= soma). `applyBankBondsHolding` mantem `manualValue = marketValueOverride ?? cost` e `costBasis = cost`. `pruneUnchangedBalanceCandidates` reaplica o candidate de Bank Bonds enquanto `bondMarketValues` estiver vazio no holding (garante que o mapa por-bond popule no primeiro sync pos-deploy mesmo se a soma nao mudou).

**Custo do agregado alinhado ao Position Performance (v1.13.1):** o `costBasis` do holding usava `computeBankBondsPrincipal` (ticker-agnostico, subtrai cada resgate ao preco de venda) — divergia do Total Cost da tab Position Performance ($10.453 vs $10.504 no caso do usuario) porque a Position Performance soma o `totalCost` por-ticker (subtrai resgate ao custo MEDIO). Novo helper `computeBankBondsCost` (`src/lib/bankBonds.js`) = soma de `computeBankBondsMarketValue(...).byTicker[*].totalCost` (mesma fonte que a Position Performance usa), agora passado como `principal` nos 3 call sites de `applyBankBondsHolding` — os dois numeros batem. As duas funcoes divergem so quando um bond comprado fora do par e parcialmente resgatado.

**Delta padronizado como os tickers auto (v1.13.2):** o delta ao lado do valor no card "US Bank Bonds" mostrava `+$XXX (+X.XX%)`; padronizado pro mesmo tratamento que o day-change dos tickers `auto` (linha ~5286 de `ManualHoldingRow`'s irmao no mesmo arquivo) — so `+X.XX%`, sem valor em `$`, sem parenteses, sempre visivel (nao respeita `valuesHidden`, igual ao day-change). O `$` bruto (`bondGainLoss`) continua calculado internamente so pra decidir a cor (verde/vermelho/neutro).

**Card "US Bank Bonds" padronizado como um ticker auto (ex: IVV) + Refresh price/Refresh all puxam do SimpleFin + Cash tambem atualiza via Refresh all (v1.13.3):** removida a linha "Cost" e "As of (SimpleFin)" do card expandido (o `costBasis` continua calculado internamente so pra derivar o delta % da linha 1, ja existente desde v1.13.1/v1.13.2 — nao aparece mais como linha propria). Adicionado o botao "Refresh price" no accordion, **reusando o mesmo bloco/estilo** do botao de `HoldingRow` (tickers auto) — `RefreshCw` + label, full-width, borda, spinner via `busy` — so que agora renderizado condicionalmente em `ManualHoldingRow` quando o pai passa a prop `onRefresh` (so a holding `bank-bonds-aggregate` recebe; Cash e outras holdings manuais nao).

- **Novo helper de modulo `syncFidelityAndFetchCandidates(auth)`** (`src/App.jsx`, perto de `fetchPrice`): dispara `POST /api/fidelity-pending?resource=sync` (admin-only no servidor — `fetch()` nao lanca em 403, entao uma chamada nao-admin so resolve com o que ja estiver staged, sem erro) e le de volta `GET /api/fidelity-pending` pra pegar `balanceCandidates`/`bondHoldings` mais recentes. Sujeito ao throttle server-side de 6h (`SYNC_THROTTLE_MS`) — chamadas dentro da janela so retornam o staging atual, sem novo fetch real no SimpleFin.
- **Nova funcao `refreshFromSimplefin()`** (`src/App.jsx`, componente `PortfolioTracker`): chama o helper acima e aplica cada `balanceCandidate` de kind `cash`/`bank-bonds` via `applyFidelityBalanceUpdate` (a mesma funcao que ja existia, antes so chamada pelo fluxo de staging do Transactions.jsx). Silenciosa em qualquer falha (nao-admin, sync nao configurado, erro de rede) — e um enhancement de fundo, nunca deve virar erro visivel na tab Holdings.
- **`refreshAll()`** agora chama `refreshFromSimplefin()` logo apos `refreshUsdBrlRate()`, ANTES do early-return por zero holdings auto — Cash/Bank Bonds sao holdings manuais e nao seriam tocadas por essa funcao senao. Chamada nao-blocking (aplica seu proprio `setHoldings` de forma independente); os dois fluxos (preco de tickers auto + SimpleFin) compoem sem conflito porque cada um so escreve campos proprios (`applyFidelityBalanceUpdate` nunca toca `costBasis`; `applyBankBondsHolding` le `existing.marketValueOverride` pra decidir `manualValue`, entao a ordem de resolucao das duas promises nao importa).
- **`refreshBankBondsFromSimplefin()`**: wrapper dedicado que liga/desliga `busyIds[BANK_BONDS_ID]` (o MESMO mapa que `HoldingRow` ja usa pra tickers auto) ao redor de `refreshFromSimplefin()`, pra alimentar o spinner do botao "Refresh price" do card Bank Bonds.
- **Cash agora atualiza via Refresh all** (nao tinha botao dedicado antes nem depois — so pelo Refresh all, como pedido) — `applyFidelityBalanceUpdate`'s branch `kind === "cash"` ja existia e ja gravava `simplefinSyncedAt`; so faltava ser chamada fora do fluxo de staging do Transactions.jsx.

**Limitacao:** como o sync real e throttled a 1x/6h no servidor, tanto "Refresh price" quanto "Refresh all" dentro da janela de 6h so re-leem o staging existente (nao buscam dado novo do SimpleFin) — mesmo comportamento de throttle que ja existia pro "Sync Fidelity" da tab Transactions, agora so exposto tambem pela tab Holdings.

Testes novos: 4 em `test/simplefin-map.test.mjs` (criacao/skip de bond buys) + 3 em `test/bank-bonds.test.mjs` (`computeBankBondsMarketValue`: match por descKey, fallback pra custo, sem metadata).

**Limitacoes:** um bond manual sem metadata cujo holding do SimpleFin parseia pra descKey geraria uma compra sintetica duplicada — mas fica em staging (visivel/descartavel), nao e escrito silenciosamente. Se a soma total do SimpleFin nao mudar entre dois syncs, o mapa por-bond nao reaplica (aceitavel — proximo sync com soma diferente reaplica).
...
**(2) Deteccao de delta de posicao para acoes/ETFs (feature nova):** nova funcao `stockPositionDeltas(account, netQtyByTicker)` em `lib/simplefin-map.js` — irma de `bondBuyTransactions` (v1.13.0), mas para holdings com `symbol !== ""` (acoes/ETFs, nao bonds). A cada sync, faz diff das `shares` reportadas pelo SimpleFin contra o net qty conhecido (`computeNetQty`, agora **exportada** de `lib/simplefin-map.js` — antes existia so como helper module-level em `src/App.jsx`, duplicada linha 395-405) e sintetiza candidatos `buy`/`sell` na MESMA fila "Trades" ja existente (reaproveita `approvePendingFid`/`discardPendingFid` — sem UI de aprovacao nova). `api/fidelity-pending.js` `handleSync` calcula `netQtyByTicker = computeNetQty(liveTx)` e passa pra `mapSimplefinPayload`.

**Regras de delta (decisoes de produto do usuario):**
- Ticker novo ou shares aumentaram -> `buy`, preco = custo medio do snapshot (`purchase_price`, fallback `cost_basis/shares`), com nota explicita de que e custo medio aproximado, **nao** o preco real da compra incremental.
- Shares diminuiram -> `sell`, preco = `market_value` como proxy, com nota de que e estimativa (o feed nao traz preco de venda real).
- Falta de preco/`market_value`, ou liquidacao total sem valor -> vao pra `unmapped` (needs review) — nunca chuta um numero.

**Idempotencia (licao tecnica importante — foi o motivo da reprovacao na rodada 1 da auditoria):** `simplefinId` durável = `sfstock-delta:<accountId>:<ticker>:<sharesNew>` — ancorado na posicao-ALVO do snapshot, **NAO** no `asOf`/balance-date. A primeira tentativa amarrava o id ao `asOf`, o que quebrava a idempotencia: o balance-date avanca a cada dia de sync mesmo sem o usuario agir, entao o mesmo delta nao resolvido ganhava id novo a cada ~6h e duplicava na fila — risco de contar posicao em dobro. Segue o mesmo padrao ja usado por `bondBuyTransactions` (`sfbond-buy:<descKey>`, tambem sem data na identidade). Usa `sharesNew` (nao `qtyDelta`) pra evitar colisao entre deltas de mesmo tamanho. Os `unmapped` derivados do delta tambem recebem esse `simplefinId` (antes ficavam `undefined` e furavam o dedupe).

**Fase 2 (objetivo final do usuario — automacao 100% sem aprovacao humana):** NAO implementada nesta feature, deliberadamente. A deteccao e pura (sem side-effect no ato de detectar; aplicacao so via aprovacao manual ja existente), deixando o caminho preparado. Orientacao do usuario: criar confianca no processo manual primeiro, so depois automatizar (mesmo padrao ja seguido por Cash/Bank Bonds no v1.7.0/commit `62161f1`).

**Limitacao conhecida documentada:** liquidacao total onde o ticker desaparece INTEIRAMENTE dos holdings de UMA conta nao e detectada como venda, porque `netQtyByTicker` e agregado entre contas e a funcao nao trackeia posicao por conta — status quo, sem regressao, documentado em comentario no codigo.

**Badges na UI:** tabela de staged Trades ganhou coluna "Notes" com badges `DELTA` (delta derivado) e `BOND BUY` (bond sintetizado, v1.13.0) pra diferenciar de trades reais do feed (que continuam sem badge).

**Testes:** `test/simplefin-map.test.mjs` ganhou 12 testes novos (total 51 passando), incluindo idempotencia cross-day. `test/perf-history.test.mjs` 11/11 sem regressao.

-----

### SimpleFin Feed — A escolha de CUSIP no dropdown de INTEREST vira bind persistente (jul/2026, v1.15.0)

**Sintoma reportado pelo usuario:** "estao aparecendo uns interests pra eu aprovar; ja selecionei os bonds equivalentes no dropdown e aprovei 2x, mas quando roda o sync ele pede novamente".

**Causa raiz (diagnostico completo antes de qualquer codigo):** a escolha do dropdown vivia so em `bondTickerEdits` (state local de `src/Transactions.jsx`), era aplicada ao `ticker` de UM evento no `approvePendingFidBond()` e em seguida **apagada**. Nao existia nenhum store "issuer -> CUSIP" — entao o proximo cupom do MESMO bond (transacao SimpleFin nova, `simplefinId` novo, data nova) chegava tao nao-resolvido quanto o anterior. Aprovar 2x nao podia ensinar nada ao app.

E o motivo de eles nunca resolverem sozinhos: a auto-resolucao do branch INTEREST (v1.9.0) exige `knownBondsByDescKey.get(descKey)`, que por sua vez exige `couponRate` + `maturityDate` + `shortName` na transacao de **compra** (`buildKnownBondsByDescKey`, `lib/bond-meta.js`). Os 7 bonds antigos do usuario (cadastrados quando a Fidelity ainda mandava CUSIP no CSV) nao tem essa metadata e **nunca** podem entrar nesse Map. O unico mecanismo que quebrava o circulo era o bind manual da UI "Bond Matching" — **removida na v1.13.0** (`0e3da48`). Desde entao `bondBindings` so era populado pelo auto-bind do servidor, que deriva de `knownBondsByDescKey` — subconjunto estrito, informacao nova zero. Circulo fechado: nada no app conseguia aprender esses 7 bonds.

**Fix (fecha o circulo reusando o que ja existia, sem UI nova):**

- **`lib/simplefin-map.js`:** `mapOneTransaction` ganhou 4o parametro `bondBindings = {}` e `mapSimplefinPayload` a opcao `bondBindings` — **segunda fonte** de resolucao do branch INTEREST, consultada depois de `knownBondsByDescKey` (que continua tendo precedencia; valor normalizado com trim+uppercase). Ambiguidade continua intocada: 0 ou 2+ holdings casando com o issuer nao resolve nem com bind (o bind e por `descKey`, que so existe quando o match e unico).
- **`api/fidelity-pending.js`:** `handleSync` passa `bondBindings: pending.bondBindings` pro mapper — e o que faz a decisao do usuario valer nos syncs seguintes. `pending` ja era lido antes do map, sem reordenacao necessaria.
- **`src/Transactions.jsx` (`approvePendingFidBond`):** quando a linha aprovada e `kind === "interest"`, tem `descKey` e o usuario escolheu explicitamente um CUSIP que esta em `knownBankBondTickers`, o par `descKey -> CUSIP` vai junto no PATCH (`bondBindings`, merge raso server-side) e no state local. Pick sem `descKey` continua one-off **de proposito** — bindar por nome de emissor sozinho colaria o bond errado quando o mesmo emissor tem varios CDs.
- **Aplicacao imediata no resto do lote:** as linhas que sobram em staging para o MESMO `descKey` (nao marcadas, ou cupons posteriores do mesmo CD) recebem o CUSIP na hora, sem exigir que o usuario repita o pick linha a linha. Um pick explicito na linha sempre vence o bind.
- **Feedback visual (constraint iPhone — `title` de hover nao serve):** legenda de 9px abaixo do dropdown, "your pick is saved for this bond" quando ha `descKey`, "one-off — this pick can't be saved" quando nao ha, nada quando o ticker ja esta resolvido.

**Efeito cascata (a parte que realmente encerra o problema):** o bind gravado reativa o `backfillBondMetadata` (v1.11.0, ja existente e ate agora ocioso justamente para esses bonds) — ele copia cupom/vencimento/shortName/notes da description do holding do SimpleFin para as transacoes de compra daquele CUSIP. A partir dai o bond entra em `knownBondsByDescKey` sozinho e passa a resolver pelo caminho automatico: INTEREST, REDEMPTION e o accrual do Composition Evolution, todos de uma vez. O bind vira uma rede de seguranca, nao a dependencia permanente.

**Testes:** 6 casos novos em `test/simplefin-map.test.mjs` (bind resolve o que `knownBondsByDescKey` nao resolve; normalizacao trim/uppercase; precedencia de `knownBondsByDescKey` sobre bind conflitante; bind de outro bond nao vaza; ambiguidade 2+ holdings continua nao resolvendo; omitir `bondBindings` preserva comportamento anterior). Suite completa verde: 57 + 34 + 14 + 12 + 11 = 128 testes. Build verde.

**Diagnosticado e NAO corrigido nesta rodada (fora do escopo pedido, vale como divida tecnica conhecida):**

- `persist()` engole erro (nao re-lanca) e `approvePendingFidBond` segue em frente removendo as linhas do staging mesmo quando o `PUT /api/transactions` falhou (409 de concorrencia ou rede) — o evento fica nem no live nem no staging, e volta no proximo sync. Vale fazer o approve abortar quando o save falha.
- O filtro cliente de "ja esta no live" usa `${date}|${ticker}|${amount}`; como o approve TROCA o ticker pelo CUSIP, o staged (nome do issuer) nunca casa com o live. O dedupe do servidor cobre isso via `simplefinId`, o do cliente nao — deveria casar tambem por `simplefinId`.
...
|**ID sintetico de Bank Bonds sem CUSIP = `<YYYYMMDD><cupom em basis points>`, sem prefixo/separador (jul/2026, sha `8b05eed`, v1.12.0)**|Formato decidido com o usuario: vencimento com 8 digitos fixos garante ausencia de colisao por concatenacao (os 8 primeiros chars sao sempre o vencimento). Cupom obrigatorio no ID porque so o vencimento colidiria nos dados reais do usuario (dois bonds vencendo em 05/08/2029) — sem o cupom, os dois bonds gerariam o mesmo ID.|
|**Resolucao INTEREST/REDEMPTION do SimpleFin depende de `couponRate`/`maturityDate`/`shortName`, nao do formato do `ticker` (aproveitado em jul/2026, sha `8b05eed`)**|Mecanismo ja existente (v1.9.0/v1.10.0) e ticker-agnostico — bonds com ID sintetico auto-resolvem contra eventos futuros do SimpleFin sem precisar da tela "Bond Matching" nem de CUSIP real, desde que a metadata (cupom/vencimento/emissor) esteja preenchida na transacao.|
|**Compra de bond criada pelo sync usa custo = `purchase_price x shares` do SimpleFin (jul/2026, v1.13.0)**|O feed do SimpleFin traz `purchase_price` e `shares` por holding (correcao do usuario sobre a suposicao inicial de que o custo nao vinha) — `bondCostBasis(h)` usa o produto, com `cost_basis` e depois `market_value` como fallbacks. Representado a par (`price=1000`/`qty=cost/1000`) pra bater com a representacao do CSV. Gain/loss real cai de (market value − custo); so o fallback pra market value comeca em ~0.|
|**Current value de Bank Bonds casado por bond, com fallback pro custo (jul/2026, v1.13.0)**|`computeBankBondsMarketValue` usa o market value por-bond do SimpleFin (keyed por descKey), nao a distribuicao proporcional-por-custo do total agregado (v1.8.0). Bond que o feed nao retorna cai pro proprio custo (gain/loss 0) em vez de sumir do total ou receber uma estimativa proporcional enganosa.|
|**Sync cria bond buys em staging, nao escreve direto (jul/2026, v1.13.0)**|As compras sintetizadas vao pra `pendingFid` e o usuario aprova como qualquer trade — mesmo padrao de redemptions/income. Serve de rede de seguranca contra duplicata (bond manual sem metadata cujo holding parseia): fica visivel e descartavel, nunca escrito silenciosamente.|
|**Delta de posicao de acoes/ETFs ancorado no `simplefinId = sfstock-delta:<accountId>:<ticker>:<sharesNew>`, nao no `asOf` (jul/2026, PR draft #138)**|Ancorar no `asOf`/balance-date (tentativa inicial, reprovada na auditoria) quebrava a idempotencia: o balance-date avanca a cada sync mesmo sem acao do usuario, entao o mesmo delta nao resolvido ganhava id novo a cada ~6h e duplicava na fila de aprovacao — risco de contar posicao em dobro. Ancorar na posicao-alvo (`sharesNew`) segue o mesmo padrao ja usado por `bondBuyTransactions` (`sfbond-buy:<descKey>`).|
|**Fase 2 (auto-aprovacao de deltas de posicao) deliberadamente adiada (jul/2026, PR draft #138)**|Orientacao explicita do usuario: criar confianca no processo manual (varias semanas de sync real aprovado a mao) antes de automatizar — mesmo caminho ja percorrido por Cash/Bank Bonds (v1.7.0) antes de virar auto-apply.|
...
|**Accrual estimado de Bank Bonds removido de todo o Income History/KPIs — so dado real conta (jul/2026, merge `00988bc`, v1.14.1)**|Decisao explicita do usuario: valores de dividendo/interest exibidos nos cards devem refletir so o que realmente foi pago e importado (SimpleFin `bondIncome` + Fidelity CSV), nunca uma projecao. `accrueByFreqAsEvents` e o step 3 de `buildBondEvents` foram deletados em `Dividends.jsx`; `Performance.jsx` teve sua copia reduzida ao mesmo criterio. Efeito aceito: valores CAEM para bonds com cobertura incompleta de pagamentos reais.|
|**Bond Projections removido — nenhuma estimativa de juros de bond permanece no app (jul/2026, merge `038d6fa`, v1.14.2)**|Na v1.14.1 o card foi mantido por ser forward-looking (cupons futuros, badge "EST" proprio, fora de qualquer total de "recebido"). O usuario reviu a decisao no mesmo dia e pediu a remocao: a regra passa a ser simples e sem excecao — o app so mostra juros de bond realmente pagos e importados (SimpleFin `bondIncome` ou CSV Fidelity). Removido o card, `buildBondProjections` e todos os helpers que ficaram orfaos; `buildBondEvents` unificou a assinatura com a copia de `Performance.jsx`.|
...
|**Evento com pay date incerto e descartado, nunca contado contra a ex-date (`api/dividends.js`, jul/2026, merge `77b4186`, v1.16.3)**|O fallback anterior (`payDateUncertain: true` + `date = exDate`, merge `d0054d4`) reintroduziu o mesmo problema que tentava evitar: a ex-date quase sempre ja passou, entao o guard de "data futura" nunca disparava e o evento contava como recebido nos totais mesmo sem o dinheiro ter caido. Validado contra extrato real da Fidelity: 4 eventos fantasmas ($233.69) nao existiam no extrato. Decisao: melhor faltar um dado incerto do app do que exibir um dado errado como se fosse confirmado — `continue` no loop principal exclui o evento por completo, telemetria via `meta.payDateUncertainSkipped`.|
...
|**Heartbeat de sync SimpleFin usa `id` estavel, removido explicitamente ao voltar a saude (jul/2026, Fase 3, commit `fbc9a15`)**|Diferente dos demais tipos de alerta (id embute a data, "some" sozinho quando o dia muda), o heartbeat representa um ESTADO continuo ("sync com problema"), nao um evento pontual — precisa de UM alerta persistente enquanto a condicao existir. `id` fixo (`simplefin_heartbeat`) evita duplicar entradas por dia; quando o sync volta a ficar saudavel nao ha "novo id" que substitua o antigo, entao o codigo remove explicitamente via `filter(a => a.id !== HEARTBEAT_ID)`.|
|**`api/ingest-fidelity.js` + `docs/plans/scraper/` deletados na Fase 3 (jul/2026, commit `fbc9a15`)**|Scraper Playwright/TOTP nunca foi ativado em producao (sem `INGEST_TOKEN` setado) — superado pelo SimpleFin Bridge desde a Fase 1. Deletar libera 1 slot de Serverless Function (11/12 no limite do Vercel Hobby) para features futuras. `constantTimeEqual` (usado pelo endpoint deletado) ja vivia em `lib/auth.js` desde o hardening batch — nada ficou orfao.|
|**Auto-apply de Balance Updates (Cash/Bank Bonds) via SimpleFin, sem Approve/Dismiss manual (jul/2026, commit `62161f1`)**|Decisao explicita do usuario: abrir mao da revisao manual especificamente para esses dois updates — Trades e Income do card "Fidelity Import" continuam manuais. Reduz friccao de um fluxo que na pratica sempre era aprovado sem edicao.|
...
|**`src/lib/bankBonds.js` como fonte unica de verdade do replay Bank Bonds (jul/2026, sha `400568e`)**|`computeBankBondsPrincipal` (App.jsx) e `computeBankBondsValueAt` (Performance.jsx) eram implementacoes independentes do mesmo calculo, uma ticker-agnostica e outra agrupada por CUSIP — a divergencia entre as duas gerava os 4 numeros diferentes que o usuario via para o mesmo holding. Consolidar num modulo `src/lib/` compartilhado por ambos os arquivos elimina a possibilidade estrutural de re-divergirem.|
|**Sem Current Value (Bank Bonds) => Current Value = Total Cost, gain/loss zero (jul/2026, sha `400568e`)**|Decisao explicita do usuario: quando `marketValueOverride` do SimpleFin nao esta disponivel, mostrar um numero neutro (sem juros projetados) e mais honesto do que reintroduzir a projecao sintetica de juros que causava a divergencia original no card Position Performance.|
|**Resolucao de ticker de redemption via `descKey` (issuer+coupon%+maturity), so para bonds com CSV importado (jul/2026, sha `400568e`)**|SimpleFin nao traz CUSIP nas redemptions — so a `description`. Comparar essa description contra `couponRate`/`maturityDate`/`shortName` (extraidos do CSV Fidelity via `extractBondMeta`, ja usado por `Transactions.jsx`) e o unico sinal disponivel para religar a venda a compra certa. Bonds sem esses metadados (inseridos manualmente) ficam sem match — aceito, degrada sem quebrar, mesmo padrao do restante do sistema Bank Bonds.|
|**`applyFidelityBalanceUpdate` auto-cria `bank-bonds-aggregate` em vez de descartar o update (jul/2026, sha `fe72fbe`)**|Descartar silenciosamente (toast invisivel, sem usuario olhando) era pior que criar o holding com um valor incompleto (so `manualValue` do primeiro sync, sem transacoes reais ainda). Um holding com dado parcial e visivel/corrigivel; um update descartado em background e invisivel e nunca corrigido sozinho.|
|**`derivedFromTransactions: false` protege o holding auto-criado de ser zerado ate a primeira transacao real (jul/2026, sha `fe72fbe`)**|`applyBankBondsHolding` roda em todo load/refresh e recalcularia `manualValue` como 0 (sem transacoes Bank Bonds ainda) se nao houvesse guard — apagando o valor recem-trazido do SimpleFin. Checagem estrita (`=== false`, nao truthy) preserva o comportamento de holdings antigas com o campo `undefined`.|
|**Janela de 44 dias do sync (`SIMPLEFIN_WINDOW_DAYS`) mantida sem aumento (jul/2026, sha `fe72fbe`)**|Aumentar a janela buscaria eventos mais antigos (interest/redemption), mas custa mais chamadas a Bridge do SimpleFin, com risco de acionar advisory de cap de uso. Decisao explicita do usuario: documentar como limitacao conhecida em vez de mexer.|
|**`lib/bond-meta.js` na raiz do repo, nao `src/lib/` (jul/2026, sha `747fcc2`)**|`extractBondMeta`/`buildKnownBondsByDescKey` precisam rodar tanto no client (`src/Transactions.jsx`) quanto em `api/fidelity-pending.js` (serverless). `src/lib/` e importado hoje so por componentes React; um modulo puro compartilhado entre client e servidor vive em `lib/` (mesmo padrao ja usado por `lib/simplefin-map.js`). `src/lib/parsing.js` mantem `extractBondMeta` como re-export para zero mudanca de contrato nos callers existentes.|
|**Resolucao automatica de ticker de INTEREST so em match unico (1 holding bate + 1 CUSIP conhecido), nunca por aproximacao (jul/2026, sha `747fcc2`)**|0 ou 2+ matches deixam o ticker como o issuer bruto (status quo, nunca pior que antes) em vez de arriscar um match errado — mesma filosofia conservadora ja usada pela resolucao de redemption via `descKey` (sha `400568e`). Um match errado silencioso seria pior que nenhuma resolucao.|
|**Dropdown de CUSIPs conhecidos (nao texto livre) no card Fidelity Income para corrigir ticker de interest (jul/2026, sha `747fcc2`)**|Texto livre permite erro de digitacao que criaria um ticker novo inexistente em vez de vincular a posicao real. Dropdown restringe a escolha aos CUSIPs de Bank Bonds ja cadastrados pelo usuario.|
|**Checagens de formato de ticker (regex de CUSIP) espalhadas pelo codigo sao um risco sistemico ao trocar o esquema de identidade (jul/2026, sha `8b05eed`, v1.12.0)**|Rodada 1 da auditoria passou build + testes mas o auditor pegou 2 bugs de perda silenciosa de dado: os guards de REDEMPTION/INTEREST em `src/lib/parsing.js` exigiam `CUSIP_RX` (9 chars) no ticker recuperado via `knownBondsByDescKey`, descartando silenciosamente tickers sinteticos (11 chars) — juros nunca contabilizados, posicao nunca zerada no vencimento. Build/testes nao pegam esse tipo de drop porque o codigo e sintaticamente valido, so o dado errado desaparece. **Ao introduzir um novo esquema de ID, procurar TODAS as validacoes que assumem o formato antigo (grep por `CUSIP_RX` / `[A-Z0-9]{9}`)** antes de considerar a feature completa — nao basta o caminho feliz (criacao do ID) funcionar, os consumidores downstream (guards, filtros, alertas) tambem assumem o formato antigo em pontos nao obvios.|
...
- **Bank Bonds — matching de redemption via `description` do SimpleFin (jul/2026, sha `400568e`):** ⚠️ **Nao validado.** A resolucao de ticker de bonds resgatados compara `descKey` (issuer+coupon%+maturity) contra a `description` que o feed SimpleFin retorna, assumindo que ela e igual (ou proxima o suficiente) a "Symbol Description" do CSV Fidelity para o mesmo bond. Nunca confirmado com um par real observado. **Validar apos o primeiro sync real pos-deploy:** Holdings "valor principal" (Total Cost em Bank Bonds) deve bater com Position Performance "Total Cost" na mesma linha, e "Market Value (SimpleFin)" deve bater com "Current Value".
- **Bank Bonds — matching de INTEREST via issuer + holdings do sync (jul/2026, sha `747fcc2`):** ⚠️ **Nao validado.** Mesma pendencia de fundo da linha acima, aplicada a resolucao de ticker de transacoes INTEREST: assume que o issuer da description de INTEREST bate (apos normalizacao) com a description do HOLDING correspondente no mesmo payload de sync. Validado apenas contra fixtures sinteticas em `test/simplefin-map.test.mjs`. **Validar apos o primeiro sync real pos-deploy:** conferir na tab Dividends/Bond Projections se pagamentos de interest aparecem vinculados ao CUSIP correto (nao ao nome do issuer) para bonds ja cadastrados; para bonds ainda sem CUSIP conhecido, conferir se o dropdown manual no card "Fidelity Income" aparece como esperado.
...
- **Duas implementacoes independentes do mesmo calculo divergem silenciosamente ao longo do tempo (jul/2026, sha `400568e`)** — `computeBankBondsPrincipal` (App.jsx) e `computeBankBondsValueAt` (Performance.jsx) nasceram como calculos separados do mesmo replay de transacoes Bank Bonds (PR #86 vs PR #93), um ticker-agnostico e outro agrupado por CUSIP. Nenhum dos dois estava "errado" isoladamente, mas o agrupamento por ticker quebrava silenciosamente quando o pseudo-ticker de uma redemption do SimpleFin nao batia com a compra original — gerando 4 numeros diferentes na UI para o mesmo holding, sem nenhum erro/warning ate a auditoria manual. Regra: quando dois arquivos precisam do mesmo calculo sobre a mesma fonte de dados (transactions), extrair para um modulo compartilhado desde o inicio, mesmo que os dois usos pareçam levemente diferentes (ticker-agnostico vs por-ticker) — a duplicacao com pequenas variacoes e o cenario mais perigoso de re-divergencia, mais dificil de flagar em code review do que uma copia identica.
- **Pseudo-chaves derivadas de texto livre (description) sao uma fonte de matching fragil que precisa de fallback documentado, nao de solucao perfeita (jul/2026, sha `400568e`)** — o SimpleFin nao expoe CUSIP nas redemptions de Bank Bonds, so a `description` em texto livre. A resolucao via `descKey` (issuer+coupon%+maturity) cobre o caso onde o CSV Fidelity ja foi importado (mesmos metadados disponiveis dos dois lados), mas nao ha garantia formal de que a description do SimpleFin seja byte-identica ao "Symbol Description" do CSV — ficou documentado como pendencia de validacao pos-deploy em vez de assumido como resolvido. Regra: ao integrar com uma fonte externa que so fornece texto livre como identificador, documentar explicitamente o nivel de confianca do match (e o que acontece quando falha) em vez de tratar a heuristica como definitiva.
- **Toast de erro em fluxo automatico (background, sem usuario olhando) e equivalente a descartar silenciosamente (jul/2026, sha `fe72fbe`)** — `applyFidelityBalanceUpdate` mostrava um toast de erro quando `bank-bonds-aggregate` nao existia ainda, mas o auto-apply do SimpleFin roda em background (load da pagina, sync automatico) — ninguem esta olhando a tela pra ver o toast. Um erro que so aparece num fluxo nao-supervisionado e, na pratica, um descarte silencioso. Regra: para qualquer acao automatica/background, o caminho de falha precisa se auto-corrigir (aqui: auto-criar o holding faltante) ou persistir de forma visivel (ex: `unmapped`, Alerts) — nunca so um toast efemero.
- **Um filtro silencioso (`!isFinite(mv)`) escondido dentro de outro filtro legitimo (`symbol !== ""`) e dificil de flagar em code review (jul/2026, sha `fe72fbe`)** — `computeBalanceCandidates` tinha dois `continue`/skip no mesmo loop de soma de Bank Bonds: um correto (excluir acoes/ETFs) e um bug (excluir `market_value` invalido sem sinalizar). Os dois pareciam parte da mesma logica de "filtrar o que nao e bond". Regra: ao auditar uma funcao de agregacao/soma, separar mentalmente cada `continue`/skip individualmente e perguntar "esse caso deveria aparecer em algum lugar visivel (unmapped/log) ou e realmente descartavel?" — nao assumir que todos os skips do mesmo loop tem a mesma justificativa.
- **Investigar 4 causas candidatas antes de corrigir evita over-fixing e sub-fixing (jul/2026, sha `fe72fbe`)** — das 4 causas levantadas para "bonds sumindo", 1 nao precisava de mudanca de codigo (so copy/comunicacao), 1 foi decisao consciente de nao mexer (trade-off documentado), e so 2 eram bugs reais. Corrigir as 4 sem esse diagnostico previo arriscaria mexer em codigo que nao precisava (janela de sync) ou deixar de comunicar o falso-alarme (usuario continuaria achando que compras de bond "somem"). Regra: para sintomas vagos tipo "X as vezes nao aparece", mapear TODAS as causas plausiveis e classificar cada uma (bug real / decisao de produto / falso alarme) antes de decidir o que corrigir.
- **Description de INTEREST do SimpleFin nao carrega os mesmos metadados que REDEMPTION — nao assumir shape identico entre tipos de transacao do mesmo feed (jul/2026, sha `747fcc2`)** — a resolucao de ticker de redemption via `descKey` (sha `400568e`) funcionava porque a description de REDEMPTION trazia coupon%/maturity suficientes para montar a chave. INTEREST do mesmo feed SimpleFin so traz o nome do issuer, sem esses campos — copiar a mesma abordagem direto teria falhado silenciosamente. Fix precisou de uma fonte de dado auxiliar (os HOLDINGS do mesmo sync, que tem description completa) para religar issuer a CUSIP. Regra: ao reaproveitar um padrao de resolucao/matching entre dois tipos de evento de uma mesma API externa, verificar explicitamente se o shape do dado de entrada e realmente equivalente antes de assumir que a mesma logica serve.
- **Consolidar 2 implementacoes duplicadas de um helper no MESMO commit que resolve o bug que elas causam, nao depois (jul/2026, sha `747fcc2`)** — `buildKnownBondsByDescKey` ja existia duas vezes (privada em `src/lib/bankBonds.js`, inline no `ImportModal` de `Transactions.jsx`) antes desta feature. Em vez de adicionar uma terceira copia para o novo consumidor (`api/fidelity-pending.js`), a extracao para `lib/bond-meta.js` aconteceu como parte da propria correcao — reforca a licao de `computeBankBondsPrincipal`/`computeBankBondsValueAt` (sha `400568e`, ver acima): duplicacoes do mesmo calculo divergem silenciosamente: melhor consolidar no momento em que um terceiro consumidor apareceria, nao adicionar mais uma copia.
- **Identidade de dedup ancorada em data/balance-date, mesmo quando parece estavel, reintroduz o item a cada janela de sync (jul/2026, PR draft #138, achado da rodada 1 do auditor)** — o `simplefinId` de delta de posicao de acoes/ETFs (`stockPositionDeltas`) foi inicialmente ancorado no `asOf` do snapshot; como o balance-date avanca a cada sync mesmo sem o usuario agir, o mesmo delta nao resolvido gerava um id novo a cada ~6h e duplicava permanentemente na fila de aprovacao — risco de contar a mesma posicao em dobro se ambas as linhas fossem aprovadas. Mesma familia de bug do `appliedBalanceIdsRef` (commit `62161f1`, ver acima): qualquer chave de idempotencia derivada de "quando" em vez de "o que" (posicao-alvo, valor, conteudo) tende a reintroduzir o item a cada nova execucao periodica. Fix: ancorar no dado que so muda quando a posicao real muda (`sharesNew`), replicando o padrao ja usado por `bondBuyTransactions` (`sfbond-buy:<descKey>`, tambem sem data na identidade). Regra: ao desenhar um `simplefinId`/chave de dedup para qualquer novo tipo de evento sintetizado a partir de um snapshot periodico, perguntar explicitamente "essa chave muda so por causa do relogio, ou so quando o dado que ela representa muda de verdade?" antes de aprovar o design.
- **Logica duplicada entre `Dividends.jsx` e `Performance.jsx` sem badge visual em uma das copias inflou numeros silenciosamente por varios PRs (jul/2026, merge `00988bc`, v1.14.1)** — `buildBondEvents` (eventos de bond interest) existia duplicada nos dois arquivos desde o PR #93 ("DIV TTM/YoC% para Bank Bonds na Position Performance"). A copia de `Dividends.jsx` sempre teve o badge "EST" nas linhas estimadas do Dividend History, deixando visivel pro usuario o que era projecao. A copia de `Performance.jsx` (alimentando `Div TTM`, `Total Div`, `YoC %` e o `totalReturn` do grafico) nunca teve equivalente visual — o accrual estimado inflava esses numeros sem nenhuma indicacao. Regra: quando a mesma logica e duplicada por convencao do projeto (arquivos separados para reduzir blast radius), toda superficie de UI que consome o resultado precisa do MESMO tratamento visual de incerteza — nao so a copia "principal"; se uma copia nao pode ter o mesmo indicador visual (ex: um KPI numerico sem espaco pra badge), isso e um sinal de que a estimativa nao deveria estar ali para comecar.
- **Decisao manual do usuario que nao e persistida vira pedagio recorrente — e remover a UI que gravava o dado confirmado pode deixar o mecanismo de auto-resolucao girando em falso (jul/2026, v1.15.0)** — o dropdown de CUSIP em linhas de INTEREST (v1.9.0) aplicava a escolha a UM evento e descartava o state; como cada cupom novo e uma transacao SimpleFin nova, o app pedia a mesma resolucao para sempre (~7 bonds x 12 cupons/ano). Pior: a UI "Bond Matching" (v1.10.0), unica forma de gravar um bind `descKey -> CUSIP` que nao derivasse de `knownBondsByDescKey`, foi removida na v1.13.0 por parecer redundante — deixando `bondBindings` populado so pelo auto-bind, que deriva do proprio `knownBondsByDescKey` (subconjunto estrito, informacao nova zero) e o `backfillBondMetadata` da v1.11.0 ocioso exatamente para os bonds que precisavam dele. Duas regras: **(1)** toda decisao manual do usuario que resolve uma ambiguidade recorrente precisa ser gravada em algum store durável, com uma chave estavel (aqui `descKey`, nao o nome do emissor) — senao a feature vira toil infinito; **(2)** antes de remover uma UI de confirmacao manual, mapear se ela e a UNICA porta de entrada de algum dado — se for, o pipeline que consome esse dado passa a rodar sempre vazio, silenciosamente.
- **Um fallback "para nunca perder o dado" pode reintroduzir o bug que a versao anterior tentava resolver, se o valor de fallback escapa do guard que deveria bloquea-lo (jul/2026, merge `77b4186`, v1.16.3)** — o fix do PR/merge `d0054d4` adicionou `payDateUncertain: true` justamente para nao esconder silenciosamente casos sem pay date resolvida, mas manteve `date = exDate` como fallback pro campo usado no guard de "data futura" (`if (date > todayISO) continue`). Como a ex-date de um dividendo declarado quase sempre ja passou (o intervalo tipico ex-date -> pay-date e de poucas semanas), esse guard nunca disparava para esses eventos — eles eram contados como recebidos nos totais/KPIs mesmo quando a pay-date real (desconhecida) ainda nao tinha chegado. So havia um badge visual "EX-DATE" na UI de auditoria (Dividend History), que nao afetava nenhum calculo agregado. Regra: quando um campo de fallback e usado tanto para exibicao (badge/aviso) quanto para um guard de logica (aqui, comparacao de data), verificar que o guard realmente cobre o caso de incerteza — um valor "quase sempre no passado" nunca vai disparar um guard de "esta no futuro", entao a mitigacao real precisa ser excluir o dado, nao so sinalizar.
- **Validar contra o extrato real da corretora, linha a linha, e mais confiavel do que ler o codigo e assumir que os guards existentes cobrem o caso (jul/2026, merge `77b4186`)** — o bug de dividendos fantasmas so foi confirmado comparando a saida do app com um CSV real da Fidelity (Accounts History, jun/2026): todos os eventos com pay date resolvido bateram centavo a centavo, e os 4 que nao bateram eram exatamente os `payDateUncertain: true`. Regra: para qualquer feature de calculo financeiro (income, totais, KPIs), ter um extrato real de referencia para comparar e o teste mais forte disponivel — mais forte do que revisar a logica isoladamente, porque expõe efeitos colaterais de guards que parecem corretos no papel mas nunca disparam na pratica (ver licao acima).
- **Sessao interrompida por limite de API e retomada por outra sessao deve ter o teste com dado inconsistente tratado como um achado real de auditoria, nao um detalhe de implementacao (jul/2026, sha `747fcc2`)** — a implementacao original parou no meio por limite de sessao do Claude Code; a sessao que retomou encontrou 1 teste cuja chave `descKey` esperada tinha um trecho extra (" CD") que nao correspondia ao que `extractBondMeta` de fato extrai da description usada no mesmo teste — corrigido antes de mandar para auditoria. Regra: ao retomar trabalho de uma sessao anterior interrompida, rodar a suite de teste e tratar qualquer discrepancia entre o valor esperado no teste e o comportamento real da funcao como um bug a investigar (nao ajustar o teste as cegas para "fazer passar").
