...
- **Dividend History** (auditoria): tabela colapsavel com todo historico de pagamentos (Date · Ticker · $/Share · Qty · Total), ordenada por data desc, scroll vertical. Quarto card — apos "Dividends Monthly Y/Y".
  - **Badge "EX-DATE" (jul/2026, merge `d0054d4`):** exibido na coluna Ticker quando `e.payDateUncertain === true` — cor ambar `#e0a458` (mesmo padrao visual que o badge "EST" de bond interest estimado tinha nesta tabela ate jul/2026 v1.14.1, quando o badge "EST" foi removido do `DividendHistoryTable` junto com o accrual — ver "Bank Bonds interest em todos os cards" acima; hoje "EST" so aparece no card Bond Projections). Tooltip complementar explica que a data exibida e a ex-date, nao a pay-date real confirmada. Agregados (Income History chart, KPIs All Time/YTD, Position Dividends) foram deixados intocados de proposito — o `totalReceived` e o mes de bucketing continuam corretos mesmo quando a granularidade exata do dia e incerta; so a UI de detalhe (Dividend History) precisa do aviso.
