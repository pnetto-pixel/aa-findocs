...
**Unico lugar do app que usa estimativa de juros de bond (desde jul/2026, v1.14.1 — ver "Bank Bonds interest em todos os cards" acima).** Forward-looking por design (projeta pagamentos futuros ainda nao ocorridos), sempre com badge "EST" proprio, e nunca soma em nenhum total de "dividendo recebido" dos demais cards. Card colapsável adicionado como 5º card na tab. Função pura `buildBondProjections(transactions, bondIncome, freqByCusip, todayISO, nMonths=12)`:
...
- **`FREQ_DAYS` module-level compartilhada (bugfix jul/2026 — commit d93cd87; historico, ver nota abaixo):** o mapa frequencia→dias, que so `buildBondProjections` usava corretamente, foi promovido a constante module-level em cada arquivo (`Dividends.jsx`, `Performance.jsx`) e passou a ser reaproveitado tambem por `buildBondEvents` (accrual do Income History), que antes ignorava a frequencia calibrada e fatiava sempre por mes de calendario — ver "Bug fix — Bond Interest estimate gerava pagamentos fantasma" abaixo. **Superado em jul/2026 (v1.14.1):** com o accrual removido de `buildBondEvents`, `FREQ_DAYS` deixou de ser usado nesse consumidor. Em `Dividends.jsx` permanece module-level (ainda usada por `buildBondProjections`, que continua vivo); em `Performance.jsx` foi deletada como codigo morto junto com o resto do accrual (`buildBondProjections` nao existe nesse arquivo).
...
|**Bond Projections mantido como unico consumidor de estimativa de juros de bond (jul/2026, v1.14.1)**|E card explicitamente forward-looking (projeta cupons futuros ainda nao pagos) com badge "EST" proprio, nunca soma em nenhum total de "recebido" — natureza diferente do accrual retroativo removido dos demais cards, que fingia ser historico real sem deixar isso claro o suficiente em todo lugar (Performance.jsx nao tinha badge nenhum).|
