...
|**Indicador de versao no header via `VERCEL_GIT_COMMIT_SHA` injetado pelo Vite (jul/2026)**|Apos um deploy que "nao rolou" (sem como distinguir deploy ausente de cache stale do Safari iOS), o header ganhou um label `commit · data-do-build` a esquerda do botao "Refresh all". Fonte: `define: { __APP_BUILD__ }` em `vite.config.js` lendo `VERCEL_GIT_COMMIT_SHA` (env que a Vercel injeta em todo build) — zero manutencao manual, sem bump de package.json. Build local/dev mostra "dev". **Workflow pos-deploy: comparar o hash exibido no app com `git log origin/main` — se diferem, o deploy nao aconteceu ou o browser esta servindo cache velho (force-reload).**|
