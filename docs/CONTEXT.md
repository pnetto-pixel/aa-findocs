...
|**Qty Fidelity Bank Bonds dividida por 1.000 no parser (PR #86 + follow-up, item 40)**|Fidelity registra a Quantity de CDs/bonds como valor de face em dolares (1000 = um CD de $1.000), com o Price ($) ja correto por unidade. `tx.qty = qtyAbs / 1000` converte para contagem de unidades; preco intocado. Logica isolada: so aplica quando `assetClass === "Bank Bonds"` (CUSIP); outros ativos intocados. O diagnostico original do PR #86 (preco x10) estava errado e foi revertido — transacoes Bank Bonds importadas com a versao antiga devem ser apagadas e re-importadas.|
