...
// Brazilian B3 stocks: brapi.dev (BRL price) + Frankfurter (USD/BRL FX)
...
async function fetchUsdBrlRate() {
  // Frankfurter: free, no key. Returns BRL per 1 USD.
...
async function handleBrazilian(ticker, brapiKey) {
  ...
  // Get USD/BRL rate
  const brlPerUsd = await fetchUsdBrlRate();
    ...
    try {
      ...
      if (sr.ok) {
        ...
        if (match?.description) {
          // Strings come back like "VANGUARD TOTAL STOCK MARKET ETF" — title-case it lightly
          name = match.description
            .toLowerCase()
            .replace(/\b\w/g, (c) => c.toUpperCase());
        }
        if (!industry && match?.type) {
          // ETF, Common Stock, etc.
