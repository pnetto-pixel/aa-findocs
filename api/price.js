...
async function fetchUsdBrlRate(brapiKey) {
  // Try brapi first — real-time quote
  if (brapiKey) {
    try {
      const url = `https://brapi.dev/api/v2/currency?currency=USD-BRL&token=${encodeURIComponent(brapiKey)}`;
      const r = await fetch(url);
      if (r.ok) {
        const d = await r.json();
        const item = Array.isArray(d?.currency) ? d.currency[0] : null;
        const rate = parseFloat(item?.bidPrice ?? item?.askPrice);
        if (rate && rate > 0) return rate;
      }
    } catch (e) {}
  }
  // Fallback: Frankfurter (ECB, daily EOD)
...
async function handleBrazilian(ticker, brapiKey) {
  ...
  // Get USD/BRL rate (real-time via brapi, fallback Frankfurter)
  const brlPerUsd = await fetchUsdBrlRate(brapiKey);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isBrazilianTicker(tickerRaw)) {
      payload = await handleBrazilian(tickerRaw, brapiKey);
    } else {
